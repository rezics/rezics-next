...
if ((await fuseki.commandHealth()).moduleVersion !== '0.5.7') {
  throw new Error('Fuseki command module 0.5.7 is required; run yarn toolchain:install and restart the stack');
}
...
try {
  ...
  if (activated) {
    ...
    const offlineIndexDigest = digest(JSON.stringify({ family: 'jena-textindexer-v1',
      image: 'rezics/fuseki:6.2.0-cmd0.5.7',
