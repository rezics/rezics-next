#!/usr/bin/env python3
"""Validate one bounded text Contribution candidate with pinned Jena SHACL."""
...
PROFILE = Path(__file__).resolve().parents[1] / "definitions" / "text-contribution-v1.ttl"
SHAPE = "https://rezics.com/definition/text-contribution-v1/contribution-shape"
PROFILE_SHA256 = "4a42b1851eaa3cace3e06c5a142329e7c3c380e756123c3b131dc2584c9d2d8b"
SAFE_IRI = re.compile(r"^https://rezics\.com/id/[0-9a-f-]{36}$")


def validate(data, contribution, jena_home, java_home, temp_root):
    if not SAFE_IRI.fullmatch(contribution):
        raise ValueError("invalid Contribution focus")
    if data.stat().st_size > 65536:
        raise ValueError("Contribution candidate exceeds 64 KiB")
    shapes = PROFILE.read_bytes()
    if hashlib.sha256(shapes).hexdigest() != PROFILE_SHA256:
        raise ValueError("text-contribution-v1 shape digest changed without profile review")
    env = os.environ.copy()
    ...
    with tempfile.TemporaryDirectory(prefix="contribution-shape-", dir=temp_root) as dirname:
        shape_file = Path(dirname) / "shapes.ttl"
        shape_file.write_bytes(shapes +
            f"\n<{SHAPE}> <http://www.w3.org/ns/shacl#targetNode> <{contribution}> .\n".encode())
    ...
    return {"profile": "text-contribution-v1", "profile_sha256": PROFILE_SHA256,
            "jena_shacl": version, "contribution_focus": contribution,
...
def main():
    ...
    report = validate(args.data.resolve(strict=True), args.contribution,
