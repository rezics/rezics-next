#!/usr/bin/env python3
"""Validate one bounded Main Version default selection candidate with pinned Jena SHACL."""
...
PROFILE = Path(__file__).resolve().parents[1] / "definitions" / "main-default-selection-v1.ttl"
SHAPE = "https://rezics.com/definition/main-default-selection-v1/selection-shape"
PROFILE_SHA256 = "92f91c7e990901a457bd688dd47f576178ffa1daa90f35a4d2d1c53f751de52a"
...
def validate(data, selection, jena_home, java_home, temp_root):
    ...
    if hashlib.sha256(shapes).hexdigest() != PROFILE_SHA256:
        raise ValueError("main-default-selection-v1 shape digest changed without profile review")
    env = os.environ.copy()
    ...
    return {"profile": "main-default-selection-v1", "profile_sha256": PROFILE_SHA256,
