...
| --- | --- | --- |
| Semantic graph | Resource/type facts, NameRecords, titles/labels, predicate identities/definition revisions, typed assertion values, identified relation occurrences, roles/qualifiers, version-scoped provenance, Tag/Path structure, sparse substantive Realm decisions, exact published revision references, semantic revisions, receipts and outbox. | No account secrets or independently edited document bodies. Direct accepted edges are derived from full assertions; compiled shapes are versioned artifacts, not a second editable model. |
...
| Classification | Preserve Sense, Application, decision and context identities. Index admitted accepted relationships; a Tag Path ancestor becomes a match only through an explicit versioned expansion rule. Concept, Path and Sense are not interchangeable keyword IDs. |
