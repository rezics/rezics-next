...
| Fuseki | `rezics/fuseki:6.2.0-cmd0.5.26`, built locally | G-029; selected native and real API checks pending | Adds the separate immutable aggregate-default policy profile while keeping earlier profile digests unchanged. Rebuild with `yarn toolchain:install`; the new tag prevents Compose from reusing the earlier profile inventory. |
