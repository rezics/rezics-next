...
| `yarn package:npm-oracle` | Check installed npm 11.19.1 and its bundled Arborist, then compare fixed caller-supplied lockfile-v3 fixtures with native `npm ls --package-lock-only --all --json --long --offline --ignore-scripts` and Arborist's virtual-tree edges. Use isolated configuration/cache under `.temp/package-npm-oracle/`, retain exact input bytes, native paths, versions, peer hosts, source/integrity and errors. No registry requests, installation, lifecycle scripts or artifact fetches. |
