...
| `yarn package:go-checksum-oracle` | Recheck the pinned Go archive through `package:go-oracle`, then use Go 1.27.1 `go mod download -json` with explicit public proxy and checksum database settings for fixed `golang.org/x/sync@v0.1.0`. Compare Go's verified `GoModSum` to the h1 value computed from a fresh bounded capture; verify the lookup's signed tree note, fetch bounded fixed-origin tiles and check record inclusion. A separate fresh-cache `go list -m -json` checks whether the native metadata-only path fetches a module archive. Store diagnostic metadata under `.temp/package-go-checksum/`; this does not make the Main API a checksum-database verifier without monotonic trust state. |
