...
| `yarn stack:clone --profile qa --run-id <source> --persistent --to-run-id <target>` | Candidate load-fixture operation: after the source stack has stopped, verify its retained source/model/schema/analyzer/engine fingerprint, then copy its entire PostgreSQL, Fuseki TDB2/Lucene and RustFS named volumes and immutable object directory into a new isolated QA project. Use the pinned PostgreSQL image as the copy helper. Preserve source owner credentials and graph lineage while assigning fresh loopback ports. Reject a live source or an existing target. The command leaves the target running for exact-query and fresh-command-cohort checks; it is not yet a qualified 10,000-Work fixture path. |
...
| `yarn load [--works 10000 --duration 180 --seed-workers 1] [--keep]` | Runs the separate OPS05/SEARCH07/SEARCH18/SEARCH19 mixed host profile in an isolated persistent QA Compose project, resetting its named volumes on completion unless `--keep` retains them for a stopped-state clone experiment. Product-command corpus creation, cold/warm public queries, selected-Work native-delta proof, private Contribution adapter/restart probe, mixed reads/writes, engine restart checks and k6 metrics remain in the profile. Seed workers are bounded to 1–4 and default to one; each Work retains its authorized create, draft, publish and select order. Compare small diagnostic runs before using parallel seeding for qualification. Artifacts record the worker count under `.artifacts/load/<run-id>/`; this does not extend the 30-minute QA budget or gate every backend batch. Smaller runs cannot qualify the named 10,000-Work host profile, but may evidence their actual assertions. The command still seeds every Work online; reusable snapshot setup and full growth instrumentation remain implementation work. |
