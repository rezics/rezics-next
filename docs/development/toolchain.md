...
| PostgreSQL recovery utilities | Host PostgreSQL 18.6: `pg_basebackup`, `pg_verifybackup`, `pg_ctl` | Adopted for isolated OPS03 QA fault fixtures | The fault tier may take a physical backup of only its own disposable QA project's PostgreSQL cluster, verify it and start a distinct local replay copy under `.temp/` while the source is fenced. These utilities are invoked by the registered `yarn test`/`yarn qa` case; they never target the development or practical-load project. Host and container major/minor versions match. |
