...
| `yarn stack:up [--profile dev\|qa --run-id <id> [--persistent]]` | Starts the Compose project for local services and prints generated endpoints. QA is disposable tmpfs by default; `--persistent` gives an isolated QA project named volumes for offline recovery drills. Use the same options for `stack:down` and `stack:reset`; reset removes its volumes. A saved project cannot switch storage mode. |
| `yarn stack:logs [--profile dev\|qa --run-id <id> [--persistent]]` | Prints a bounded tail of service logs for startup and health diagnostics; options must match the saved project. |
| `yarn stack:status [--profile dev\|qa --run-id <id> [--persistent]]` | Shows the current service state and health for a saved local project; options must match the saved project. |
...
| `yarn load [--works 10000 --duration 180]` | Runs the separate practical OPS05/SEARCH18/SEARCH19 host profile in an isolated persistent QA Compose project, resetting its named volumes on completion: product-command corpus creation, cold/warm public queries, mixed reads and admitted writes, Fuseki/PostgreSQL restart checks, and k6 metrics. Artifacts live under `.artifacts/load/<run-id>/`; it does not extend the 30-minute `yarn qa` budget. Smaller options are diagnostics, not acceptance evidence. |
...
| Fuseki | `rezics/fuseki:6.2.0-cmd0.5.10`, built locally | Adopted | TDB2 + jena-text with the REZICS command module and generated shapes; cmd0.5.10 validates focused SHACL and exact binding checks against a read-only union of the selected named graphs instead of copying every triple into a new graph per command. This removes the measured corpus-size copy cost in the 1,057-Work practical seed while retaining the generated focus and native invariant gates; native equivalence and a 1k/2k seed benchmark qualify the change. It retains cmd0.5.9's per-JVM public-search write seqlock, cmd0.5.7's maintenance quarantine/rebuild and process-instance UUID, cmd0.5.6's normal caller capability and exact head CAS, cmd0.5.5's maintenance capability and cmd0.5.4's Content MatchUnit and eligibility checks. This host's BuildKit/Compose path has kept serving already tagged Podman images after rebuild; a changed module uses a fresh image tag. |
