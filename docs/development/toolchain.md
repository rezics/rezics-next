...
| `yarn search:rebuild [--job <uuid>]` | On the stopped-writer development stack, persistently quarantines public search, replays the exact Content cut, stops Fuseki, runs the pinned `jena.textindexer` against the named volume, restarts Fuseki, verifies RDF/Lucene/source membership and activates a new index generation. A failed or interrupted run retains the quarantine and job ID for retry. This initial operator command does not target QA tmpfs or production. |
