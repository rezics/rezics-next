...
| `yarn load [--works 10000 --duration 180 --seed-workers 1]` | Runs the separate practical OPS05/SEARCH07/SEARCH18/SEARCH19 host profile in an isolated persistent QA Compose project, resetting its named volumes on completion: product-command corpus creation, cold/warm public queries, one selected-Work read-after-write native-delta proof, an internal private Contribution posting/adapter/restart probe, mixed reads and admitted writes, Fuseki/PostgreSQL restart checks, and k6 metrics. Seed workers are bounded to 1–4; the default is serial, and every worker preserves each Work's authorized create, draft, publish and select sequence. Compare small diagnostic runs before using parallel seeding for qualification. Artifacts record the worker count and live under `.artifacts/load/<run-id>/`; the profile does not extend the 30-minute `yarn qa` budget. Smaller Work counts or durations are diagnostics, not acceptance evidence. |
