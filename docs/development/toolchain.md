...
| Fuseki | `rezics/fuseki:6.2.0-cmd0.5.6`, built locally | Adopted | TDB2 + jena-text with the REZICS command module and generated shapes; cmd0.5.6 requires a separate caller capability for every non-maintenance command and adds transaction-local exact Work/selection-head CAS bound to receipt identity and Access scope. It retains cmd0.5.5's maintenance capability and cmd0.5.4's Content MatchUnit, public eligibility and historical binding checks. This host's BuildKit/Compose path has kept serving already tagged Podman images after rebuild; a changed module uses a fresh image tag. |
