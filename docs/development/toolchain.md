...
| Fuseki | `rezics/fuseki:6.2.0-cmd0.5.5`, built locally | Adopted | TDB2 + jena-text with the REZICS command module and generated shapes; cmd0.5.5 requires a per-stack maintenance capability for bootstrap, restore and retained-zero receipt prefixes. It retains cmd0.5.4's fixed Content MatchUnit projection, narrowly admitted public MatchUnit shape validation, public eligibility linkage checks and historical binding outcomes. This host's BuildKit/Compose path has kept serving already tagged Podman images after rebuild; a changed module uses a fresh image tag. |
