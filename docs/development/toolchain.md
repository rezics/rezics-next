...
| `yarn package:cargo-oracle` | Check the installed Rust/Cargo 1.98.1 binary, then run the fixed Cargo resolver 2 snapshot against a disposable loopback-only local registry and isolated `CARGO_HOME` under `.temp/package-cargo-oracle/`. Compare selected package/source/version/role edges and feature activation with the bounded REZICS profile; retain the native metadata/tree and comparison result there. The command never uses the public registry, installs artifacts, or runs build scripts. |
