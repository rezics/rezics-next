...
| Fuseki | `rezics/fuseki:6.2.0-cmd0.5.14`, built locally | Adopted, build pending | TDB2 + jena-text with the REZICS command module and generated shapes; cmd0.5.14 adds a private MatchUnit graph and separately indexed body field, with a private-write epoch for bounded read fencing. It retains cmd0.5.13's transaction-derived, 64-entry public MatchUnit delta journal and fenced exact-subject Lucene proof. The module refuses the public shortcut if a standard write operation is installed alongside the native command, so the QA raw-update assembler continues using the full inventory. This host's BuildKit/Compose path has kept serving already tagged Podman images after rebuild; a changed module uses a fresh image tag. |
