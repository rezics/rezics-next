...
| `yarn load [--works 10000 --duration 180 --seed-workers 1]` | Runs the separate OPS05/SEARCH07/SEARCH18/SEARCH19 mixed host profile in an isolated persistent QA Compose project, resetting its named volumes on completion: product-command corpus creation, cold/warm public queries, selected-Work native-delta proof, private Contribution adapter/restart probe, mixed reads/writes, engine restart checks and k6 metrics. Seed workers are bounded to 1–4 and default to one; each Work retains its authorized create, draft, publish and select order. Compare small diagnostic runs before using parallel seeding for qualification. Artifacts record the worker count under `.artifacts/load/<run-id>/`; this does not extend the 30-minute QA budget or gate every backend batch. Smaller runs cannot qualify the named 10,000-Work host profile, but may evidence their actual assertions. The command still seeds every Work online; snapshot/bulk setup and full growth instrumentation remain implementation work. |
...
| Fuseki | `rezics/fuseki:6.2.0-cmd0.5.15`, built locally | Adopted, native build passed | TDB2 + jena-text with the REZICS command module and generated shapes; cmd0.5.15 admits the atomic Main default selection's paired selection/Main Version head transition and verifies both exact successors against the receipt and revision anchors. This repairs the cmd0.5.14 `one head transition required` rejection. It retains cmd0.5.14's private MatchUnit graph, separately indexed body field and private-write epoch, plus cmd0.5.13's transaction-derived public delta journal and exact-subject Lucene proof. The module refuses the public shortcut if a standard write operation is installed alongside the native command, so the QA raw-update assembler continues using the full inventory. `bun scripts/dev/cli.ts toolchain:install` built the new image and ran its Maven tests; targeted SEARCH06 integration passed. This host's BuildKit/Compose path has kept serving already tagged Podman images after rebuild; a changed module uses a fresh image tag. |
