...
| IAM06 | E Agent membership change and exact dependent grants; E private-principal consent, change and recipient read with direct principal-grant episode dependency. General group/role/Realm semantics remain P. |
...
| LIVE04-LIVE06 | E `GET /v1/sources/conversions/{base}/child-correspondences/{candidate}` for a read-only occurrence assessment; E `POST /v1/sources/correspondences` for an explicit ambiguous source-only child match; E `POST /v1/works/{id}/source-support/withdrawal` for a recorded title binding only; P general `POST /v1/sources/withdrawals`, multi-source support and child withdrawal. |
...
| PKG01-PKG06 | E `POST /v1/package-resolutions` and E `GET /v1/package-resolutions/{resolution}` for bounded Go snapshots; E `POST /v1/package-resolutions/cargo` and E `GET /v1/package-resolutions/cargo/{resolution}` for bounded exact Cargo resolver 2 v1/v2/v3 snapshots, including `links` conflict and admitted-lock yanked eligibility; E `POST /v1/package-sources/go`, E `GET /v1/package-sources/go/{capture}`, E `POST /v1/package-sources/go/{capture}/verify` and E `GET /v1/package-sources/go-verifications/{verification}` for the bounded captured Go path; P other native ecosystems and full resolver/provider/installation behavior. |
| PKG07-PKG13 | E Go and Cargo package-resolution create/read routes above for bounded incomplete/unsupported/budget outcomes; Cargo v2/v3 also has selected native `links` unsatisfiability and v3 lock/index checksum inconsistency. P other ecosystem profiles and general unsatisfiable/timeout outcomes. |
