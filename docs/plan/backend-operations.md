...
| PKG07-PKG13 | E `POST /v1/package-resolutions`, E `GET /v1/package-resolutions/{resolution}`, E `POST /v1/package-resolutions/cargo` and E `GET /v1/package-resolutions/cargo/{resolution}` for bounded incomplete/unsupported/budget outcomes; Cargo v2/v3 also has selected native `links` unsatisfiability and v3 lock/index checksum inconsistency. P other ecosystem profiles and general unsatisfiable/timeout outcomes. |
