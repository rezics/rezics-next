...
| Next action | Complete the in-flight P0.8 two-stage text rebuild and scalable complete query/readiness profile, merge their tested worktrees, then run centralized `yarn qa`. The clean-tree full run `20260924t202630-48ca4a` passed all seven tiers in 82.5 seconds, with 1 acceptance ID passed, 17 partial and 259 uncovered. Expand P0.4 case coverage and the 10,000-Work practical load before B1. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
...
| P0.4 | Shared-stack `yarn qa` core under `tests/qa/` and `scripts/qa/`, starting with real Account signup/session and Main/Fuseki readiness. OPS01/IAM01 have partial smoke coverage; later increments add oracles, split cases, model/fault/e2e/load tiers, fixture fetcher and `--record`. | `yarn qa` passes within 30 minutes on this host; setup is shared, overlapping full runs on one checkout are rejected, failure reruns preserve their partial scope, and `--record` verifies and records in one run. Any failure can be rerun by ID. | Full clean-tree `20260924t202630-48ca4a` passed static, unit, integration, model, fault/recovery, browser and load in 82.5 seconds. The harness preserves nested failed-test selection and exact seeded property replay; the hash-checked Q42 fixture and product-command Main/Realm selection oracle are merged. Scoped Biome/import gates and negative fixtures run in `yarn check`. Restore/crash cases, broader case coverage and a passing full clean-tree `--record` remain. |
