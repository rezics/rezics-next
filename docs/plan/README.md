...
| P0.2 | Java command endpoint, assembler/image wiring and transactional gate under `infra/jena/`; Main command adapter and writer migration under `services/main/`. SYS02/SYS09/SYS10/SYS14 and model validity cases are affected. | The module gate in the toolchain lock passes; Main has no `execFile` validator path. | Partial: module 0.3.0 bounds update templates, enforces current-focus and canonical-shape coverage, validates own receipt and +1 sequence with matching unique outbox in the transaction, and bounds bootstrap/restore transitions. Product assembler rejects raw updates; QA fixtures use a separate assembler. Isolated gate tests and Main Work/restore probes passed; merged `yarn qa` run `20260924t180407-c27d85` passed. Maintenance receipt prefixes still lack caller capability authentication; exact domain-head CAS and external Access admission remain Main-owned. |
| P0.3 | TypeScript model IR and compiler for the 12 existing profiles, with generated shapes, TypeBox schemas, types, constants, registry and arbitraries. | Recorded candidate outcomes reproduce; the Python validators and hand-written profile Turtle are deleted; `yarn gen:check` is clean. | Partial: all 12 authored profiles generate byte-identical Turtle with unchanged digests, plus JSON-LD contexts, node-local TypeBox schemas/types, vocabulary, focus roles and seeded fast-check candidates. Sixty-six recorded candidates are now TypeScript fixtures. An isolated 0.2 native run matched 55/66 outcomes; 11 invalid candidates committed, and 26 reports omitted the expected violation path. The native binding/reporting gate and retirement of old Turtle/Python remain. |
| P0.4 | Shared-stack `yarn qa` core under `tests/qa/` and `scripts/qa/`, starting with real Account signup/session and Main/Fuseki readiness. OPS01/IAM01 have partial smoke coverage; later increments add oracles, split cases, model/fault/e2e/load tiers, fixture fetcher and `--record`. | `yarn qa` passes within 30 minutes on this host; setup is shared, overlapping full runs on one checkout are rejected, failure reruns preserve their partial scope, and `--record` verifies and records in one run. Any failure can be rerun by ID. | Partial: merged run `20260924t181510-4c4f94` passed static, unit, shared-stack integration and a real Toxiproxy lost-response fault in 21.5 seconds, with 6 partial IDs and 271 uncovered. Full-case promotion requires declared coverage. Model, e2e and load tiers remain uncovered; fault/recovery still lacks restore/crash cases. |
