...
| Next action | Qualify the in-flight P0.8 10,000-Work practical load, merge its worktree, then run full `yarn qa` at that batch boundary. The rebuild/search batch full run `20260924t210338-81320a` exposed three test-fixture failures; the repaired failed-case rerun `20260924t210718-c40e19` passed, but cannot certify the changed full tree. Complete the positive offline Lucene rebuild and paired-owner activation drill, expand P0.4 case coverage, then start B1. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
...
| P0.4 | Shared-stack `yarn qa` core under `tests/qa/` and `scripts/qa/`, starting with real Account signup/session and Main/Fuseki readiness. OPS01/IAM01 have partial smoke coverage; later increments add oracles, split cases, model/fault/e2e/load tiers, fixture fetcher and `--record`. | `yarn qa` passes within 30 minutes on this host; setup is shared, overlapping full runs on one checkout are rejected, failure reruns preserve their partial scope, and `--record` verifies and records in one run. Any failure can be rerun by ID. | Full clean-tree `20260924t202630-48ca4a` passed all seven tiers in 82.5 seconds. The merged rebuild/search batch run `20260924t210338-81320a` passed static, fault/recovery, browser and load; unit, integration and model fixture failures were repaired, and `--only-failed` run `20260924t210718-c40e19` passed. The harness preserves failed-test selection, seeded replay, the hash-checked Q42 fixture and a product-command Main/Realm selection oracle. Scoped Biome/import gates run in `yarn check`. Restore/crash cases, broader case coverage and a clean full `--record` remain. |
