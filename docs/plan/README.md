...
| Status | The current management Goal started at 03:57:20 UTC on clean `f2c8aa1`. G-001 through G-017 are integrated and affected-verified on `main`; G-018 is merged with affected checks underway. RATE03 is a declared complete-case candidate with passing merged selected checks, not recorded backend qualification. The last broad historical run `20260925t151147-02d115` passed 11 of 277 mixed cases; no `yarn qa --backend --record` result exists. See the [active management program](#active-management-program) and generated [qualification page](qualification.md). |
| Next action | Verify G-018 on merged Source/Jena cmd0.5.24, then integrate G-019 npm migration 020 and run its first real API and physical recovery selection on combined source. G-020 independently works on organization-bound Realm moderation. Preserve complete field control, other package ecosystems, recovery and the remaining owner schemas; final reconstruction and recorded backend QA remain the final gate. |
...
| [G-018](../goals/tasks/G-018.md) / LIVE04 | One source-qualified Open Library author occurrence becomes an explicit native Work credit. | Worker `dd20a47` merged as `043aec3`, with combined OpenAPI guard repaired at `9ccb77f`; affected merged checks underway. Worktree API `20260926t074730-1831a8`, held-graph recovery `20260926t074730-20dab9`, native command `20260926t074739-acebf1`, growth, unit, gen and static passed. Exact external author occurrence becomes one append-only native credit; LIVE04/MODEL05/MODEL06 remain partial. Worker task `01a0dc8f-baf0-7b11-9328-a691aa18d3d7` finished. |
