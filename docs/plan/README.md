...
| Program | Phase 0 connects the [toolchain](../development/toolchain.md) and the [executable harness](../testing/test-harness.md). Retained M01–M10 backend/API batches then run before full web journeys, following the [execution program](#execution-program) and [batch cadence](execution-workflow.md#batch-cadence). |
...
| Next action | Complete the practical 10,000-Work/180-second profile `load-20260925t002559-e525e2` on fixed source `c1d78a5`; it passed 9,000 sealed Works without command errors at 02:54:22 UTC. The last 1,000 took 19m32s as the host reclaimed Fuseki file cache and used swap; the remaining seed is projected around 03:14–03:18 UTC under the 03:26 UTC profile cap, leaving a narrow margin for mixed load and restart. That source passed all seven QA tiers in `20260925t002122-58c7bb` (9 acceptance IDs passed, 20 partial, 248 uncovered). Subsequent merged source adds retained translation-link replay, MainVersion revision advancement and exact read, admitted Content publication/eligibility/comment APIs, public Main/Realm/Content paging, cmd0.5.13 native search deltas, Realm native variant recommendations, revision-disclosure checks, Access private read leases and a cmd0.5.14 native Contribution private projection candidate. The S2 real Account/Access/Main/Content API journey now includes Realm adoption/rejection, exact historical comments and paged comment reads across writes and restore; it is unrun. Graph hold release now binds an exact Content owner cut, and transport probes keep the private route closed while peer receipt and terminal recovery remain unproved. The merged QA inventory includes owner-boundary Account, Access, Work and restore cases previously run separately. After load, build the cmd0.5.14 image, run its native tests and one merged QA, then repair any failures. Re-run the practical profile on the new source for the native delta/read-after-write gate. Finish Phase 0 and the retained M01–M10 backend/API gates before full web journeys. |
...
| P0.8 | PostgreSQL Content revisions/drafts/receipts/outbox and preparation pins; common history adapters; exact-revision publication; bounded two-owner relay into RDF MatchUnits and embedded Lucene; coherent readiness and two-stage rebuild. | WORK02–03/09–10, SEARCH01–04/06–08/15–20 and OPS03/09/11–12/15–16 pass through the harness. Pin numeric whole-request budgets and practical mixed-load objectives before qualification; retain query plans, update amplification and recovery evidence. | Content CAS/history/pins, cmd0.5.12 publication/eligibility/MatchUnit and translated-Work binding, durable relay and exact reads passed the prior merged QA as partial cases. Public search admits 20,000 MatchUnits with a 512 raw-hit complete phrase bound and position/JVM-bound index audits. The 2,000-Work diagnostic passed mixed read/write and retained-data restart; the 10,000-Work run is active on its fixed source. Subsequent merged source adds retained WORK02 link replay with a graph-loss case, advancing MainVersion revisions, public Main/Realm/classified/rated and Content phrase paging, cmd0.5.13's transaction-observed bounded search delta proof, and Realm native variant recommendations. These changes await the cmd0.5.13 image build, native tests and merged QA after load. Private paging and complete P0.8 acceptance remain open. |

After Phase 0, backend/API batches follow the [dependency order](#dependency-order)
...
| --- | --- | --- |
| B1 | Complete the S2 authenticated Work/Realm/edit/search journey through real API calls and owner state, including denied, stale, concurrent and recovery outcomes. IDs from [native Work](../testing/native-work.md), [classification](../testing/classification.md), [search](../testing/search.md) and [identity/access](../testing/identity-and-access.md); required Content, Access, Account, Main and graph integrations are in scope. | In progress: real Account OAuth, Access grants, Main/Content API calls, two Realm classification decisions, distinct Realm adoption/rejection, eligible search, stale/denied writes, concurrent same-head classification decisions, exact prior Work/MainVersion/Content reads and paragraph comments are authored in one integration fixture. The comment list uses an owner snapshot cursor and tests pagination across a later write. A separate isolated restore fixture uses a real OAuth bearer before and after Account WAL restoration, a separately stopped/copied Content cluster, graph/Access restore, exact comment/body reads, a retained page cursor and post-restore revocation. Graph hold release now checks signed Content owner epoch, sequence, graph references, exact bytes and retained rows; a missing restored Content pool stays held. The real restore fixture exercises that gate with one prepared reference. Both fixtures await merged QA. Atomic cross-owner cuts, object-store verification and mixed-cut drills remain. Full web flows follow backend/API completion. |
| B2 | Establish the Access read-admission side of SEARCH11/SEARCH12 private phrase search, then bind exact Content and graph/index positions and enforce pre-match field/unit eligibility at the API. | In progress: a ten-second durable contribution read lease, generation recheck and strong-closure pending accounting are authored, with a two-registry PostgreSQL race test queued for merged QA. Native Contribution create/edit/replay project one current private body MatchUnit to a distinct Lucene field. The internal adapter checks exact source, graph unit, posting and index position. A live Jena create/edit/replay, wildcard-posting and field-isolation test is authored but unrun on the shared QA assembler; its raw update endpoint correctly makes the adapter unavailable. The next command-only practical load adds an internal adapter probe at corpus size and after storage restart; it is unrun. Loopback tests show Elysia `afterResponse` runs before body delivery. A WebSocket probe observed the server `close` callback before a half-closed client read buffered result bytes, so that callback cannot retire an aborted Access lease; a matching nonce pong followed one small result. A pinned Node HTTP probe shows `finish` is only server-to-OS handoff and a paused client can read the full 1 MiB response afterward. Maximum-size WebSocket ordering and terminal recovery remain under investigation. The HTTP route returns 503 without owner calls pending a proven delivery/cancellation fence; broader PostgreSQL Content mapping and end-to-end closure remain. SEARCH11/12 are unqualified. `admission.ts` temporarily exceeds 800 lines because command admission and the read lease share the scope/principal lock order; extract the read lease after its live behavior is qualified. |
| B3a | Begin M01 private acting-context discovery and selection over real Account/Access/Main boundaries. IAM01/IAM03/IAM04; one principal may represent several Agents, and an Agent may have several controllers without publishing the private mapping. | Isolated implementation worktree started after the B1/B2 merged snapshot; tests and API consumer are being authored. It will enter a later merged QA batch after the fixed practical load and current B1/B2 qualification. Grant mutation, full group/role composition and web flows remain subsequent work. |
| B3+ | Implement and qualify each remaining retained M01–M10 backend/API dependency through stages A–G: persistence, authority, cross-service contracts, source/package workflows and operations. The [backend map](backend-acceptance.md) and owning case files select the next batch; no required API behavior remains uncovered before W1. | Pending. |
| W1+ | Build and qualify the full web product journeys against the completed APIs, including the authenticated S2 flow and all retained frontend acceptance. Finish local full-application browser verification and the clean final `yarn qa --record`. | Waiting for B1–B3+ backend/API gates. |
