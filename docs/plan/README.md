...
| Field | Selection |
| --- | --- |
| Scope | The retained M01–M10 implementation Goal in [GOAL.md](../../GOAL.md) is paused. The current task refactors the delivery/verification strategy and reconciles documentation; it does not resume runtime work. |
...
| Authority | Current authority covers documentation, documentation checks and coherent local commits. On explicit Goal resumption, its existing implementation/local QA scope applies. Remote pushes, production publication and paid provisioning remain outside that scope. |
...
| Status | As of 2026-09-25, B3b is merged. The last full QA `20260925t070846-87a274` failed static, unit, integration and fault/recovery on `773b11c`; repairs through `7b65c98` are not fully qualified. The earlier passing `20260925t035859-06b5a7` does not qualify these changes. Both 10,000-Work load attempts failed. The [qualification page](qualification.md) remains the generated evidence owner. |
| Next action | On explicit resumption, inspect the B3b failure queue and existing repairs, reproduce remaining failures minimally, and close the merged batch through central QA. Replace repeated command-only background seeding with compatible verified fixtures; add cost contracts/counters to the affected paths. Inventory and integrate parked B4–B7 work in dependency order after the base passes. Do not start another dependent branch chain or use the failed large load as a universal prerequisite. |
...
| P0.8 | PostgreSQL Content revisions/drafts/receipts/outbox and preparation pins; common history adapters; exact-revision publication; bounded two-owner relay into RDF MatchUnits and embedded Lucene; coherent readiness and two-stage rebuild. | WORK02–03/09–10, SEARCH01–04/06–08/15–20 and OPS03/09/11–12/15–16 pass through the harness. Derive path costs, assert multi-scale work and retain plans, amplification/recovery evidence and separately scoped host objectives. | Partial cmd0.5.15 evidence covers public paging, Content history/pins, link replay, projection, rebuild and restore. The 10-Work diagnostic passed a bounded native delta and private Contribution restart probe. The two 10,000-Work attempts failed 26 and two mixed reads after approximately 166 and 174 minutes of command seeding. Repair minimal counterexamples and fixture preparation before repeating that profile. Private paging, native cost coverage and complete P0.8 acceptance remain open. |
...
| B3b | Merge the next backend boundary slice: IAM09 explicit-consent authorization-code, refresh and introspection fence; IAM01/03/04 private acting-context preference CAS and Access recovery coverage; SEARCH02/10 rejected-candidate budget; SEARCH13 named-graph sentinel; SEARCH11/12 durable private-send receipt and final position recheck; OPS03 coordinated Account/Access/Content/graph physical restore. Owners are Account, Access, Main, Content, graph and the QA integration/fault tiers. | Merged; full QA `20260925t070846-87a274` failed on `773b11c` with 0 passed IDs, 39 partial, one failed and 237 uncovered. Subsequent repairs through `7b65c98` require qualification. Inspect static/unit, Access migration and restore/OAuth failure diagnostics before the next merged run. Private delivery/search remains unqualified. |
| B4–B7 parked | Existing work on `codex/b4-backend-next`, `codex/b5-backend-integration`, `codex/b6-backend-integration` and `codex/b7-backend-integration`. | Dependent work is staged, not accepted capability. Preserve the branches; reconcile their diffs and required consumers after closing B3b, then integrate one coherent slice at a time. Do not duplicate their implementation or infer qualification from branch names or commit counts. |
