...
| Next action | Repair the P0.6 merged QA run's Storybook type error and Content selector assertion, then merge the real authenticated browser journey and rerun its affected tiers. P0.8 has a transactional Content core; its guarded publication boundary needs a native model/Jena binding before live graph qualification. P0.2 maintenance authorization and domain-head guards remain. Grow P0.4 case coverage and mixed-corpus load alongside these consumers. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
...
| P0.4 | Shared-stack `yarn qa` core under `tests/qa/` and `scripts/qa/`, starting with real Account signup/session and Main/Fuseki readiness. OPS01/IAM01 have partial smoke coverage; later increments add oracles, split cases, model/fault/e2e/load tiers, fixture fetcher and `--record`. | `yarn qa` passes within 30 minutes on this host; setup is shared, overlapping full runs on one checkout are rejected, failure reruns preserve their partial scope, and `--record` verifies and records in one run. Any failure can be rerun by ID. | Partial: merged run `20260924t185318-e7e20f` reached all seven tiers in about 44 seconds. Integration, native model, fault, built-Worker browser and empty-corpus load passed; static found two Storybook query type errors and unit found one outdated selector expectation. The selector is repaired; the web repair is in progress. Eleven IDs had partial evidence, 266 remained uncovered. Restore/crash cases, mixed-corpus load, fixture fetcher and full `--record` qualification remain. |
...
| P0.6 | `apps/web`: vinext on Workers, Eden with TanStack Query, Tailwind/SharkUI in `packages/ui`, `native-i18n`, Storybook, Playwright; sign-in through Account, one RSC Work page and one client search component. | Playwright on `wrangler dev` and the Storybook tests pass inside `yarn qa`. | Built Worker, Eden BFF, public search, RSC Work metadata, sign-in/studio shell, Storybook and two public Playwright cases are merged. QA provisions a disposable PKCE client and Access actor; its browser tier passed in run `20260924t185318-e7e20f`. Real sign-in/Create Work browser coverage is in progress; the static Storybook type repair and broader locale/authority states remain. |
