...
| Next action | P0.1's image, Compose stack and root commands are integrated; complete its clean-clone exit by P0.2's removal of Main's host Jena/Java/Python validator path. The runnable P0.4 core now exercises a shared-stack Account/Main/Fuseki path; expand it and run full `yarn qa` at the next merged batch boundary. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
...
| P0.4 | Shared-stack `yarn qa` core under `tests/qa/` and `scripts/qa/`, starting with real Account signup/session and Main/Fuseki readiness. OPS01/IAM01 have partial smoke coverage; later increments add oracles, split cases, model/fault/e2e/load tiers, fixture fetcher and `--record`. | `yarn qa` passes within 30 minutes on this host; setup is shared, overlapping full runs on one checkout are rejected, failure reruns preserve their partial scope, and `--record` verifies and records in one run. Any failure can be rerun by ID. | Partial: the first full run failed at QA stack startup because Podman rejected tmpfs ownership options. The repaired disposable stack and targeted integration tier passed; the fixed source has no full-run pass yet. Static/unit passed in the first run. Other tiers, per-file isolation, failure reruns and recording remain pending. Next: run full `yarn qa` at the next merged batch boundary, then expand coverage and harness features. |
