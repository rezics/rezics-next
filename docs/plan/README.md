...
| [G-020](../goals/tasks/G-020.md) / IAM23 | Realm-local moderation of one organization-authored selected publication, with exact admitted episode and local-only effect. | Manager implementing in an isolated Access/Work worktree from clean `88ecfde` while two workers retain G-018/G-019. Existing admitted contribution author and selected contribution/publication decision are the candidate publisher binding to prove. Combine with G-012 suspension to target a complete IAM23 candidate only after whole-row merged evidence. Moves, quota/review and paid benefits remain retained. |
