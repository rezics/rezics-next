...
| Next action | The clean merged `yarn qa` run `20260925t035859-06b5a7` passed all seven tiers on cmd0.5.15: 8 acceptance IDs passed, 30 partial, 239 uncovered. A command-only `yarn load --works 10 --duration 10` passed with 136 reads, 28 writes, no errors, one bounded native delta and no full inventory; private Contribution current/new and old-term absence survived restart. The practical 10,000-Work run `load-20260925t040916-7656b1` is in progress on unchanged `aa7adfa`. Inspect its retained load evidence and repair any failure before qualification. The next backend batch is assembled in an isolated integration worktree; merge it into `main` after the load, run central `yarn qa`, repair failures together, then continue retained M01–M10 backend/API gates before full web journeys. |
...
| P0.8 | PostgreSQL Content revisions/drafts/receipts/outbox and preparation pins; common history adapters; exact-revision publication; bounded two-owner relay into RDF MatchUnits and embedded Lucene; coherent readiness and two-stage rebuild. | WORK02–03/09–10, SEARCH01–04/06–08/15–20 and OPS03/09/11–12/15–16 pass through the harness. Pin numeric whole-request budgets and practical mixed-load objectives before qualification; retain query plans, update amplification and recovery evidence. | Cmd0.5.15 passed the native build/tests and merged QA. Public Main/Realm/Content paging, Content CAS/history/pins, translated link replay, revision reads, Realm variants, Content projection and isolated rebuild/restore have partial QA evidence. The 10-Work command-only diagnostic passed, including a bounded native delta and private Contribution restart proof. The prior cmd0.5.12 10,000-Work run failed 26 mixed reads; the new cmd0.5.15 practical 10,000-Work run is in progress. Private paging and complete P0.8 acceptance remain open. |
...
| B3a | Begin M01 private acting-context discovery and selection over real Account/Access/Main boundaries. IAM01/IAM03/IAM04; one principal may represent several Agents, and an Agent may have several controllers without publishing the private mapping. | Isolated branch `codex/b3-acting-context` has private discovery and explicit task-scoped Agent checks, an OpenAPI consumer, and a real shared-stack Account/Access/Main test with 44 assertions. Targeted IAM03 integration passed; merge and central QA follow the fixed practical load. Grant mutation, full group/role composition and web flows remain subsequent work. |
| B3b | Merge the next backend boundary slice: IAM09 explicit-consent refresh/introspection fence; IAM01/03/04 private acting-context preference CAS and Access recovery coverage; SEARCH02/10 rejected-candidate budget; SEARCH13 named-graph sentinel; SEARCH11/12 durable private-send receipt and final position recheck; OPS03 coordinated Account/Access/Content/graph physical restore. Owners are Account, Access, Main, Content, graph and the QA integration/fault tiers. | Assembled on `codex/backend-boundary-integration` while the practical load keeps `main` unchanged. Account consent, owner-cut restore, candidate-overflow and acting-context fixtures require central runtime QA; the private HTTP route remains closed while its cross-owner send gap and unresolved receipt liveness remain open. Merge after the load, run one `yarn qa` on the merged snapshot, and repair its failure queue. |
| B4 | Stage A M01 IAM04 direct-principal `work.create` authority alongside represented-Agent selection; SEARCH11/12 native private field, position and receipt boundaries; SEARCH20 public Content projection/index restoration after loss. Owners are Account, Access, Main, Content and native graph/index fault fixtures. | Assembled on `codex/b4-backend-next` after B3b, with Access direct grant and attribution schema/API/recovery tests, generated OpenAPI, a real native private-search fixture, and a changed-cut Content rebuild fault fixture. Run one central `yarn qa` after B3b qualification and repair its queue. Direct proof pins grant, attribution, Agent and principal generations; both modes reject a stale scope epoch. SEARCH11/12 retain the closed HTTP route until Content-owned private bodies, query plan, cross-owner race and socket cancellation pass. SEARCH20 remains partial pending broader source/erasure-frontier recovery. `admission.ts` remains over 800 lines while register/claim and search-lease operations share one owner; extract after live proof QA. Browser tab state, grant mutation, compound obligations, wider direct actions and represented-path generation binding remain later M01 work. |
| B5 | Extend M01 Access with private same-scope Agent group inheritance for `work.create` (IAM05/IAM36 first profile), qualify public rated-relation multiplicity (SEARCH04) and CJK Main/Realm selection (SEARCH06), and add an optional bounded seed worker profile for OPS05/SEARCH18/SEARCH19 diagnostics. Owners are Access schema/context/admission, Main native query fixtures and the practical-load runner. | Assembled on `codex/b5-backend-integration` for central QA after B4. Group mutation and selected-path claim have a real Account/Access/Main fixture; SEARCH04/06 extend native shared-stack fixtures; seed concurrency defaults to one and awaits a small 100-Work A/B diagnostic before any full-profile use. Populated reparenting is denied; public group management, impact approval, general roles and wider IAM36 qualification remain open. `admission.ts` remains over 800 lines while group proof, register/claim and search leases share its owner; extract after live proof QA. Record wall time, unfiltered QA count, load duration and fully passed IDs after qualification. |
| B6 | Extend M01 and native Work boundaries: IAM11 older/current Account and Access physical restore against retained deletion evidence; IAM33 exact represented `work.create` proof across mandate, grant, Agent and principal generations; WORK04 explicit adaptation, recording and fork derivation from an exact retained source revision. Owners are Account, Access, Main, graph/model and the integration/fault tiers. | Assembled on `codex/b6-backend-integration` for one central QA after B5. The new Work profile requires the documented cmd0.5.16 Fuseki image. IAM11 remains partial until unrelated public Work/Content survival and broader erasure custody are tested; IAM33 still needs wider authority paths; WORK04 still needs continuity decisions beyond this explicit relation. Record wall time, unfiltered QA count, load duration and newly fully passed IDs after qualification. |
| B7 | Complete SEARCH05/09's current-only public search refusal contract across all six query and page profiles, and extend IAM11's isolated Account erasure restore to retain an unrelated public Work and exact PostgreSQL Content revision. Owners are Main public API, Account, Access, Content, graph and the integration/fault tiers. | Assembled on `codex/b7-backend-integration` for central QA after B6. Declared multi-dataset and historical intents have typed 422 responses before native reads, while malformed requests remain 400. The IAM11 fixture tests public graph and Content survival in both physical cuts; historical copy sanitization and off-host custody remain open. Record wall time, unfiltered QA count, load duration and newly fully passed IDs after qualification. |
| B8+ | Implement and qualify each remaining retained M01–M10 backend/API dependency through stages A–G: persistence, authority, cross-service contracts, source/package workflows and operations. The [backend map](backend-acceptance.md) and owning case files select the next batch; no required API behavior remains uncovered before W1. | Pending. |
| W1+ | Build and qualify the full web product journeys against the completed APIs, including the authenticated S2 flow and all retained frontend acceptance. Finish local full-application browser verification and the clean final `yarn qa --record`. | Waiting for B1–B6+ backend/API gates. |
