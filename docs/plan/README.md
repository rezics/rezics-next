...
| Status | As of 2026-09-25, merged B8 passed full `yarn qa` in `20260925t102029-423253`: all seven tiers passed, with 11 passed IDs, 34 partial and 232 uncovered. This qualifies the changed batch snapshot, not the retained product scope. Both 10,000-Work load attempts remain failed. The [qualification page](qualification.md) remains the final generated evidence owner. |
| Next action | Replace repeated command-only background seeding with compatible verified fixtures, then run small varied load diagnostics before another full practical-load attempt. Continue unmet backend/API dependencies from the acceptance map. |
