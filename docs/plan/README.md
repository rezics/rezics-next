...
| Phase | S0 graph substrate verified; S1 internal Work activation and one-scope PostgreSQL Access registration verified in part. Account assertion verification, the admission bridge, edit/history and full S1 cases are next. S2–S3 and remaining stages are pending. |
...
The first [Access migration and registry](../../services/main/README.md) were exercised on a disposable PostgreSQL 18.6 cluster. Its [JUnit result](../../services/main/tests/evidence/2026-09-24-access-admission.xml) records one test, 18 assertions and no failures: admission/receipt/outbox records after registration, same-key replay and conflict, issuer/action denial, same-key concurrency, ordinary scope closure and a competing registration/closure. This covers only one-scope IAM07 admission gating. The fixture seeds verified-principal and grant records directly; it does not qualify Account verification, grant creation, strong revocation or graph receipt binding. Next executable step: implement Account OIDC assertion verification and its current enforcement bridge, then pass a registered Access admission into an authenticated Main Work command and finish same-head edit, exact history, denied/invalid and revocation cases before S1 completes. No external credential blocks the local work.
