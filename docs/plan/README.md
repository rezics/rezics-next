...
| B4a | Stage A M01 IAM04 direct-principal `work.create` authority alongside represented-Agent selection. Access owns direct grants, public attribution bindings and admitted proof; Main owns private discovery/check and command mode selection. | Isolated branch `codex/m01-direct-principal` carries schema, API, recovery coverage and real PostgreSQL fixture. Generate the changed OpenAPI artifact after merge into the installed checkout, then run one central `yarn qa` for IAM04 direct, denied, stale/revoked and concurrent selection cases. Direct proof pins grant, attribution, Agent and principal generations; both modes reject a stale scope epoch. Wider represented-path dependency generations remain later IAM33 work. The direct proof was extracted to a helper; the existing large `admission.ts` still needs the B2 follow-up split. Browser tab state, grant mutation, compound obligation evaluation and wider direct actions remain later M01 work. |
