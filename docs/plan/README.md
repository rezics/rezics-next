...
| Next action | Run merged `yarn qa` for P0.2 maintenance capability and P0.8 Content runtime/load. The isolated model tier now passes with a fresh fixture epoch, and live Content search passes through Main's HTTP route. Integrate the authenticated Content save/provenance worktree next, then two-stage rebuild and search beyond the current 100-unit complete-population cap. P0.2 domain-head/Access guards, P0.4 case coverage and B1 remain. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
