...
| Next action | Run merged `yarn qa` for the native Content search slice, then wire Content relay runtime/readiness, two-stage rebuild and mixed-corpus load. The live publication → claimed eligibility → projection → phrase-search integration passes, but the writer lacks independent rights provenance and the public route is not yet a complete product flow. P0.2 maintenance authorization/domain-head guards and P0.4 case coverage remain. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
