...
| Status | The current management Goal started at 03:57:20 UTC on clean `f2c8aa1`. G-001 through G-027 are integrated and affected-verified on `main`; G-028 Work-title control is active in an independent worktree. IAM06, IAM23, IAM24, RATE01, RATE02 and RATE03 are declared complete-case candidates with passing selected owner checks, not recorded backend qualification. The last broad historical run `20260925t151147-02d115` passed 11 of 277 mixed cases; no `yarn qa --backend --record` result exists. See the [active management program](#active-management-program) and generated [qualification page](qualification.md). |
| Next action | G-028 continues independent Work-title owner implementation. G-029's Rating question-versus-policy brief is ready for a separate owner worktree; the manager will dispatch it and serially integrate both handoffs with affected real-owner checks. Preserve all field control, ecosystem, recovery and remaining owner schema requirements; final reconstruction and recorded backend QA remain the final gate. |
...
| [G-029](../goals/tasks/G-029.md) / RATE05 | Distinguish a new Rating question/Context from an immutable aggregate-default policy revision and real default consumer. | Ready for a separate Rating worktree from verified G-025. The counterexample must change a context-default result from 7 to 5 without changing Context identity or relabeling explicit 7/5/4.5 policies; question edits create another Context. G-028 Work/Source is independent; shared Main/model artifacts enter the manager's serial integration queue. |
