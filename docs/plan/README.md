...
| Status | The current management Goal started at 03:57:20 UTC on clean `f2c8aa1`. G-001 through G-017 are integrated and affected-verified on `main`. RATE03 is a declared complete-case candidate with passing merged selected checks, not recorded backend qualification. The last broad historical run `20260925t151147-02d115` passed 11 of 277 mixed cases; no `yarn qa --backend --record` result exists. See the [active management program](#active-management-program) and generated [qualification page](qualification.md). |
| Next action | G-018 source-qualified author occurrence adoption and G-019 npm locked topology occupy separate worktrees. Preserve complete field control, other package ecosystems, recovery and the remaining owner schemas; final reconstruction and recorded backend QA remain the final gate. |
...
| [G-019](../goals/tasks/G-019.md) / PKG03 | Bounded npm lockfile-v3 nested-instance and peer-host topology against a pinned offline native oracle. | Running in independent package worktree task `01a0dc9d-2462-7df3-9090-26f2ab24f871` from clean `c2a2f55`. npm reserves Content migration 020 after G-018's 019, so integration is ordered after G-018 even though implementation is independent. Toolchain must admit the exact npm oracle before invocation. PKG03/PKG12/PKG13 remain partial until merged evidence and wider profiles. |
| [G-020](../goals/tasks/G-020.md) / IAM23 | Realm-local moderation of one organization-authored selected publication, with exact admitted episode and local-only effect. | Ready for the next independent Access/Work slot. Manager review confirmed durable contribution author as a candidate publisher binding but found generic Access admission lacks an atomic saved participation episode; its owner schema and admitted in-flight semantics must be designed before implementation. Combine with G-012 suspension to target a complete IAM23 candidate only after whole-row merged evidence. Moves, quota/review and paid benefits remain retained. |
