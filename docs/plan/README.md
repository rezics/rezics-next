...
Account/Access/Fuseki/object storage. The fixed Main Version default now selects an eligible exact draft and atomically projects one public MatchUnit. A bounded complete phrase query and public selected read are locally exercised. A fixed Space command creates a public Space and distinct Realm capability with pinned initial policies. Guarded local adoption gives two Realms independent exact selected bodies, public fallback reads and complete bounded phrase queries. Local rejection/suppression, management grant provisioning, classification, rating and broader query filters remain pending. The mixed-cut recovery drill verified one restored public MatchUnit and jena-text lookup; crash-time index rebuilding remains unqualified. WORK02/03, CTX01 and SEARCH01/03 remain partial until local rejection, classification/rating and broader query paths run together. The
...
| Space, Context, classification and Main Version | Selected first-stage foundation. | Fixed Space/Realm creation, Main default and Realm-local selection, two-Realm effective reads and bounded public phrase queries passed scoped engine/HTTP/recovery checks. Classification, ratings, suppression and complete end-to-end gates remain pending. |
