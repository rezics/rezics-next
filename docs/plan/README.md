...
| Phase | S0 graph substrate verified; S1 internal Work activation, one-scope PostgreSQL Access registration and isolated Account assertion verification verified in part. Actual Account issuer/current enforcement, the admission bridge, edit/history and full S1 cases are next. S2–S3 and remaining stages are pending. |
...
| Qualification | S0 OPS14 and substrate OPS16 passed. The internal S1 storage test passed scoped SYS02/SYS10/SYS14 cases; one-scope Access registration/ordinary closure passed scoped IAM07 checks on PostgreSQL 18.6. Product G2–G6 and complete S1–S3 remain pending. |
...
The S1 owners are [commands](../contracts/commands.md), [identity/access](../contracts/identity-and-access.md), [model profiles](../contracts/model-profiles.md), [API/events](../implementation/api-and-events.md), [graph records](../implementation/graph-records.md), [model validation](../implementation/model-profile-validation.md), [authorization bridge](../implementation/authorization-bridge.md) and [Access implementation](../implementation/access-control.md). Its exit cases include [MODEL](../testing/model-contracts.md), [IAM](../testing/identity-and-access.md) and [SYS](../testing/backend-integration.md). The first [fixed Work metadata profile](../../model/README.md) has a pinned Jena SHACL 6.2.0 helper and [executed candidate evidence](../../model/tests/evidence/2026-09-24-work-profile.json): one valid candidate conformed; removing its Work type or required MainVersion link produced violations on the explicitly selected focus. This covers only the fixed-profile portions of MODEL15/17. The [Main storage slice](../../services/main/README.md) now uses a Yarn 4.18 lockfile, Bun 1.4.2 and Elysia 2.0.0-beta.16. Its [executed JUnit result](../../services/main/tests/evidence/2026-09-24-main-storage.xml) records one live-Fuseki integration test with 26 assertions and no failures. It exercised the internal guarded metadata Work creation, matching receipt replay, conflicting digest, lost HTTP response, same-key race, stale epoch and invalid input, current graph/text binding, immutable payload bytes, outbox count and network readiness. The current API exposes health only; no browser/client can submit a product write. The `admittedScope` in the graph test is synthetic, so these are scoped storage checks, not complete SYS/G2/G3 acceptance.

The first [Access migration and registry](../../services/main/README.md) were exercised on a disposable PostgreSQL 18.6 cluster. Its [JUnit result](../../services/main/tests/evidence/2026-09-24-access-admission.xml) records one test, 18 assertions and no failures: admission/receipt/outbox records after registration, same-key replay and conflict, issuer/action denial, same-key concurrency, ordinary scope closure and a competing registration/closure. This covers only one-scope IAM07 admission gating. The fixture seeds principal and grant records directly; it does not qualify grant creation, strong revocation or graph receipt binding. Main's [Account verifier](../../services/main/README.md) checks a JWKS-signed, audience-bound bearer assertion and forces current introspection before yielding issuer/subject to Access; its [local-issuer result](../../services/main/tests/evidence/2026-09-24-account-assertion.xml) covers scoped IAM02/IAM10 rejection cases. The actual Better Auth Account issuer/introspection/session lifecycle is not implemented, so this is no cross-service Account pass. Next executable step: implement the Account issuer and live current-enforcement service, then bind a registered Access admission to a Main Work command and finish same-head edit, exact history, denied/invalid and revocation cases before S1 completes. No external credential blocks the local work.
