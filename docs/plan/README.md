...
| Next action | Complete the P0.8 runtime, native maintenance and mixed-load worktrees, then run merged `yarn qa`. Full run `20260924t194415-382a45` exposed native fixture pollution of the product integration stack; the failed integration test passes after moving native fixtures to the isolated model tier. That model tier needs a fresh fixture control epoch before the next full run. The Content eligibility writer still lacks independent rights provenance; two-stage rebuild, P0.4 case coverage and B1 remain. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
...
| P0.2 | Java command endpoint, assembler/image wiring and transactional gate under `infra/jena/`; Main command adapter and writer migration under `services/main/`. SYS02/SYS09/SYS10/SYS14 and model validity cases are affected. | The module gate in the toolchain lock passes; Main has no `execFile` validator path. | Partial: module 0.4.0 adds exact native bindings to 0.3.0's bounded update templates, current-focus/canonical-shape coverage, own receipt, +1 sequence, unique matching outbox and bootstrap/restore transitions. Product assembler rejects raw updates; QA fixtures use a separate assembler. Merged `yarn qa` run `20260924t183323-de23dd` passed. Maintenance receipt prefixes still lack caller capability authentication; exact domain-head CAS and external Access admission remain Main-owned. |
| P0.3 | TypeScript model IR and compiler for the 12 existing profiles, with generated shapes, TypeBox schemas, types, constants, registry and arbitraries. | Recorded candidate outcomes reproduce; the Python validators and hand-written profile Turtle are deleted; `yarn gen:check` is clean. | Exit check passed: all 12 generated profile digests match, and the merged native QA tier reproduced all 66 recorded candidate outcomes with zero result-path differences in run `20260924t183323-de23dd`. The 12 handwritten Turtle definitions and 24 Python validator/test execution files were removed; historical fixtures remain. MODEL17/MODEL27 retain partial acceptance status pending broader cases. |
| P0.4 | Shared-stack `yarn qa` core under `tests/qa/` and `scripts/qa/`, starting with real Account signup/session and Main/Fuseki readiness. OPS01/IAM01 have partial smoke coverage; later increments add oracles, split cases, model/fault/e2e/load tiers, fixture fetcher and `--record`. | `yarn qa` passes within 30 minutes on this host; setup is shared, overlapping full runs on one checkout are rejected, failure reruns preserve their partial scope, and `--record` verifies and records in one run. Any failure can be rerun by ID. | Full `20260924t194415-382a45` passed static, unit, model, fault/recovery, built-Worker e2e and load, but native fixture state caused the positive Content integration to fail. It was 11 partial/2 failed/264 uncovered IDs. Native command tests now run in the isolated model tier; failed integration rerun `20260924t194827-012d9f` passed. The affected model-tier diagnostic `20260924t194758-8edd58` found its control epoch must be reset after the equivalence matrix. Restore/crash cases, mixed-corpus load, fixture fetcher and full `--record` qualification remain. |
| P0.5 | Explicit response schemas, OpenAPI export, `MainApp` type export, S3 object adapter on RustFS. | The Eden and object-storage gates pass. | Partial: aws4fetch signed conditional creation, read-back checks and eight same-key writers passed against RustFS. Main Work create/edit, exact and recovery reads use the adapter when configured. All 25 Main `/v1` routes now have explicit response schemas and inferred types; `yarn gen:check` checks `generated/openapi/main/public.json`, and the standalone Eden consumer compiles and makes two actual calls. Merged run `20260924t181510-4c4f94` passed. Other semantic object consumers, backup/restore, multipart/GC and the web Eden BFF gate remain. |
| P0.6 | `apps/web`: vinext on Workers, Eden with TanStack Query, Tailwind/SharkUI in `packages/ui`, `native-i18n`, Storybook, Playwright; sign-in through Account, one RSC Work page and one client search component. | Playwright on `wrangler dev` and the Storybook tests pass inside `yarn qa`. | The merged English/Simplified Chinese shell, search, auth, Studio and exact Work page passed the built-Worker e2e and Storybook tier in full run `20260924t192555-8e0769`. Browser cases covered PKCE, denied actor, Work creation, explicit QA read grant, Chinese exact revision on mobile, locale persistence, per-browser isolation and independent search language. The selected integration run `20260924t192231-e874b4` proved exact read denied before and allowed after the Access grant. The whole QA run failed unrelated model/harness expectations; broader authority and content journeys remain. |
...
| P0.8 | PostgreSQL Content revisions/drafts/receipts/outbox and preparation pins; common history adapters; exact-revision publication; bounded two-owner relay into RDF MatchUnits and embedded Lucene; coherent readiness and two-stage rebuild. | WORK02–03/09–10, SEARCH01–04/06–08/15–20 and OPS03/09/11–12/15–16 pass through the harness. Pin numeric whole-request budgets and practical mixed-load objectives before qualification; retain query plans, update amplification and recovery evidence. | Transactional Content draft CAS, immutable revisions, exact reads, pins, receipts and outbox are merged. Jena command module 0.5.4 validates publication, public eligibility and MatchUnit contracts while retaining all 66 historical native outcomes. Main's guarded publication settles a real Content pin and replays it; the authorized exact Content read verifies bytes and digest. Selected integration `20260924t194240-845261` proved fail-closed projection before eligibility, then a real claimed Access admission, native eligibility decision, MatchUnit projection and exact revision public phrase result. This is partial WORK10/SEARCH19 evidence. Runtime wiring, coherent readiness/rebuild, independent rights provenance, revocation and mixed load remain. |
