...
| Next action | Merge the P0.8 Content publication and native model/Jena profile branches, then run the next full `yarn qa` after the P0.6 browser repair. The authenticated Work creation journey and Storybook browser checks now pass in targeted QA; their last full run failed on a stale sequence-zero search assertion, which is repaired. P0.2 maintenance authorization/domain-head guards, P0.4 case coverage and mixed-corpus load remain. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
