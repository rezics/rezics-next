...
| Next action | While G-024 and G-025 implement separate owner slices, repair the stale Main API contract test fixture/route count exposed in G-022's isolated broad check, then merge worker handoffs serially and repeat affected real-owner checks. Preserve all field control, ecosystem, recovery and remaining owner schema requirements; final reconstruction and recorded backend QA remain the final gate. |
