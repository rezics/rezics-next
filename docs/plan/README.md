...
| B27+ | Continue remaining M01–M10 backend/API dependencies through stages A–G: persistence, authority, cross-service contracts, source/package workflows and operations. The [backend map](backend-acceptance.md) selects later batches; no required backend behavior is waived. |
