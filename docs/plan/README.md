...
| Phase | S0 graph substrate verified; S1 first authenticated fixed-profile Work HTTP command and one-scope Access/graph fence verified in part. Same-head edit, exact history, restart/restore and full S1 cases are next. S2–S3 and remaining stages are pending. |
...
| Qualification | S0 OPS14 and substrate OPS16 passed. S1 tests passed scoped SYS02/SYS10/SYS14, one-scope IAM07 claim/strong-seal cases and a partial G3 Account/Access/Main/Fuseki HTTP path on Fuseki 6.2.0/PostgreSQL 18.6. Complete G2–G6 and S1–S3 remain pending. |
