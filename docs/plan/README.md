...
| Next action | Complete P0.2's native command invariant checks for receipt, sequence, outbox and epoch guards, then integrate P0.3's generated model outputs and P0.5's object adapter. Expand P0.4's case-level evidence alongside those consumers. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
...
| P0.2 | Java command endpoint, assembler/image wiring and transactional gate under `infra/jena/`; Main command adapter and writer migration under `services/main/`. SYS02/SYS09/SYS10/SYS14 and model validity cases are affected. | The module gate in the toolchain lock passes; Main has no `execFile` validator path. | Partial: module 0.2.0 bounds update templates, enforces current-focus and canonical-shape coverage, and the product assembler rejects raw updates; QA fixtures use a separate assembler. Invalid-profile receipts retain admission metadata. Isolated product and QA image tests passed, and merged `yarn qa` run `20260924t174519-5cb33b` passed with six partial IDs. The module still accepts caller-generated SPARQL without proving exact domain-head, epoch, sequence or outbox invariants. |
| P0.3 | TypeScript model IR and compiler for the 12 existing profiles, with generated shapes, TypeBox schemas, types, constants, registry and arbitraries. | Recorded candidate outcomes reproduce; the Python validators and hand-written profile Turtle are deleted; `yarn gen:check` is clean. | Partial: all 12 authored profiles generate byte-identical Turtle with unchanged digests, plus JSON-LD contexts, node-local TypeBox schemas/types, vocabulary, focus roles and seeded fast-check candidates. The model workspace is wired into `yarn check` and QA unit selection; merged run `20260924t174819-97dfa0` passed. Native Jena candidate equivalence and retirement of old Turtle/Python sources remain. |
