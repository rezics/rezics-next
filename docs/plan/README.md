...
| Next action | Run one merged `yarn qa` on the clean cmd0.5.14 source, repair failures in one queue, then run a small command-only private/delta diagnostic before repeating the practical 10,000-Work profile. The fixed-source cmd0.5.12 run `load-20260925t002559-e525e2` sealed all 10,000 Works in 166 minutes with no command errors, but did not qualify: 26/2,326 mixed public reads returned `503 search_index_unavailable` during successful admitted writes; it stopped before persistent restart. All 395 writes succeeded and relay lag drained. The exact readiness/deadline cause is unproved; the new native bounded delta path targets this mixed-write pressure. Artifacts are retained under `.artifacts/load/`, and the isolated stack reset. Cmd0.5.14 built with its native tests after repairing a Lucene accessor and the typed test fixture. The preceding fixed source passed all seven QA tiers in `20260925t002122-58c7bb` (9 acceptance IDs passed, 20 partial, 248 uncovered). Subsequent merged source adds retained translation-link replay, MainVersion revision advancement and exact read, admitted Content publication/eligibility/comment APIs, public Main/Realm/Content paging, native search deltas, Realm native variant recommendations, revision-disclosure checks, Access private read leases and a native Contribution private projection candidate. The real Account/Access/Main/Content API journey and exact Content recovery cut await merged QA. The private search HTTP route remains closed pending a proven transport and terminal protocol. Continue Phase 0 and all retained M01–M10 backend/API gates before full web journeys. |
