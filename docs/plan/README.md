...
| Next action | Complete the practical 10,000-Work/180-second profile `load-20260925t002559-e525e2` on fixed source `c1d78a5`; it passed 5,000 sealed Works without command errors at the latest checkpoint. That source passed all seven QA tiers in `20260925t002122-58c7bb` (9 acceptance IDs passed, 20 partial, 248 uncovered). Subsequent Main source adds retained translation-link replay, MainVersion revision advancement and exact read, admitted Content publication/eligibility APIs, public Main/Realm/Content paging, cmd0.5.13 native search deltas, Realm native variant recommendations, revision-disclosure checks and an Access private read-lease foundation. The S2 real Account/Access/Main/Content API journey includes Realm adoption/rejection and is unrun. The merged QA inventory now includes the owner-boundary Account, Access, Work and restore cases previously run separately. After load, build the new image, run its native tests and one merged QA, then repair any failures. Re-run the practical profile on the new source for the native delta/read-after-write gate. Finish Phase 0 and the retained M01–M10 backend/API gates before full web journeys. |
...
| --- | --- | --- |
| B1 | Complete the S2 authenticated Work/Realm/edit/search journey through real API calls and owner state, including denied, stale, concurrent and recovery outcomes. IDs from [native Work](../testing/native-work.md), [classification](../testing/classification.md), [search](../testing/search.md) and [identity/access](../testing/identity-and-access.md); required Content, Access, Account, Main and graph integrations are in scope. | In progress: real Account OAuth, Access grants, Main/Content API calls, two Realm classification decisions, distinct Realm adoption/rejection, eligible search, stale/denied writes, concurrent same-head classification decisions and exact prior Work, MainVersion and Content reads are authored in one integration fixture. A separate isolated restore case now asserts the exact revision API returns a recovery hold, then serves retained bytes after release; it awaits merged QA. Exact comments and a single journey across the restore boundary remain. Full web flows follow backend/API completion. |
| B2 | Establish the Access read-admission side of SEARCH11/SEARCH12 private phrase search, then bind exact Content and graph/index positions and enforce pre-match field/unit eligibility at the API. | In progress: a ten-second durable contribution read lease, generation recheck and strong-closure pending accounting are authored, with a two-registry PostgreSQL race test queued for merged QA. The private index/projection, delivery adapter and end-to-end fence are absent; SEARCH11/12 remain uncovered. `admission.ts` temporarily exceeds 800 lines because command admission and the new read lease share the scope/principal lock order; extract the read lease after its live behavior is qualified. |
