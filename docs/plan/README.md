...
| Next action | P0.1's image, Compose stack and root commands are integrated; complete its clean-clone exit by P0.2's removal of Main's host Jena/Java/Python validator path. P0.4 now inventories 277 retained acceptance IDs and runs the shared-stack smoke path; add case-level oracles and missing tiers before qualification. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
...
| Main commands | Guarded Work create/edit, immutable manifests and exact history reads, receipts, RDF outbox relay with checkpoints. | SYS02, SYS04, SYS05, SYS09, SYS10, SYS12, SYS14, G3 | Validation runs through Python and a JVM per request; P0.2 replaces it. |
...
| --- | --- | --- | --- |
| P0.1 | Service/image topology in `infra/dev/` and `infra/jena/`; root commands and host process wiring in `scripts/dev/` and root `package.json`. OPS01/OPS14/OPS16 and the existing Main/Account integration path are affected. | From a clean clone, `yarn toolchain:install && yarn dev` serves healthy Main and Account. No `REZICS_*` host paths remain. | Partial: install and Fuseki image build passed; Podman-socket Compose startup, Fuseki `ASK`, Main/Account readiness and teardown passed. Six focused contract tests and the bootstrap `yarn check` passed. `yarn dev` still requires existing host Jena/Java paths for Main validation; P0.2 removes them. Full OPS01/14/16 acceptance remains pending in the expanding `yarn qa` harness. |
| P0.2 | Current batch: Java command endpoint, assembler/image wiring and transactional gate under `infra/jena/`; Main command adapter and writer migration under `services/main/`. SYS02/SYS09/SYS10/SYS14 and model validity cases are affected. | The module gate in the toolchain lock passes; Main has no `execFile` validator path. | In progress after the P0.1 stack landed. Java and Main modules have disjoint worktrees. Next: agree exact envelope/status/receipt protocol, implement and test, then run centralized qualification. |
| P0.3 | TypeScript model IR and compiler for the 12 existing profiles, with generated shapes, TypeBox schemas, types, constants, registry and arbitraries. | Recorded candidate outcomes reproduce; the Python validators and hand-written profile Turtle are deleted; `yarn gen:check` is clean. | In progress. The first increment packages all 12 reviewed Turtle shapes with checked digests and a TS profile registry for P0.2's first consumer. The authored IR, TypeBox/types/arbitraries, equivalence tests and retirement of handwritten Turtle/Python remain pending. Next: convert constraints into authored TS IR and prove candidate outcomes through the command module. |
| P0.4 | Shared-stack `yarn qa` core under `tests/qa/` and `scripts/qa/`, starting with real Account signup/session and Main/Fuseki readiness. OPS01/IAM01 have partial smoke coverage; later increments add oracles, split cases, model/fault/e2e/load tiers, fixture fetcher and `--record`. | `yarn qa` passes within 30 minutes on this host; setup is shared, overlapping full runs on one checkout are rejected, failure reruns preserve their partial scope, and `--record` verifies and records in one run. Any failure can be rerun by ID. | Partial: full `yarn qa` run `20260924t170514-85194b` passed static, unit and shared-stack integration tiers on a stable source snapshot. After correcting the smoke test's ID mapping, targeted integration run `20260924t170613-e1ea95` passed and reports OPS01/IAM01 partial, 275 uncovered. No case is certified; model, fault/recovery, e2e and load tiers remain uncovered. `--only-failed <run-id>` now reruns failed tests without borrowing earlier passes. Next: add case-level oracles, per-file isolation and remaining tiers, then use `--record` for final qualification. |
