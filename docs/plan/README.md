...
| Next action | Repair position movement in concurrent public search without weakening corruption detection, measure the 10,000-Work practical load on persistent QA storage, merge those worktrees, then run full `yarn qa` at that batch boundary. The positive offline Lucene rebuild and paired-owner activation passed in full run `20260924t212240-12bdb2`; its sole integration failure was a shared-stack test population assertion, repaired by commit `171a103` and passed in failed-case rerun `20260924t212635-ed4c53`. That partial rerun cannot certify the changed full tree. Expand P0.4 complete-case coverage, then start B1. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
...
| P0.2 | Java command endpoint, assembler/image wiring and transactional gate under `infra/jena/`; Main command adapter and writer migration under `services/main/`. SYS02/SYS09/SYS10/SYS14 and model validity cases are affected. | The module gate in the toolchain lock passes; Main has no `execFile` validator path. | Partial: native module 0.5.7 adds fenced Content rebuild commands and per-JVM identity to the private-capability and exact in-transaction domain-head CAS gate. Main claims Access grants before dispatch. The repaired native model case passed in `20260924t210718-c40e19`; broader native command families and external Access binding still need qualification. |
...
| P0.4 | Shared-stack `yarn qa` core under `tests/qa/` and `scripts/qa/`, starting with real Account signup/session and Main/Fuseki readiness. OPS01/IAM01 have partial smoke coverage; later increments add oracles, split cases, model/fault/e2e/load tiers, fixture fetcher and `--record`. | `yarn qa` passes within 30 minutes on this host; setup is shared, overlapping full runs on one checkout are rejected, failure reruns preserve their partial scope, and `--record` verifies and records in one run. Any failure can be rerun by ID. | Full clean-tree `20260924t202630-48ca4a` passed all seven tiers in 82.5 seconds. The latest merged full run `20260924t212240-12bdb2` passed static, unit, model, fault/recovery, browser and load; one shared-stack integration fixture failed on an absolute population assertion, and its repaired `--only-failed` run `20260924t212635-ed4c53` passed. The native and browser WORK01 identities now check metadata-only creation, but the complete case has not been declared. The harness preserves failed-test selection, seeded replay, the hash-checked Q42 fixture and a product-command Main/Realm selection oracle. Scoped Biome/import gates run in `yarn check`. Restore/crash cases, broader case coverage and a clean full `--record` remain. |
...
| P0.8 | PostgreSQL Content revisions/drafts/receipts/outbox and preparation pins; common history adapters; exact-revision publication; bounded two-owner relay into RDF MatchUnits and embedded Lucene; coherent readiness and two-stage rebuild. | WORK02–03/09–10, SEARCH01–04/06–08/15–20 and OPS03/09/11–12/15–16 pass through the harness. Pin numeric whole-request budgets and practical mixed-load objectives before qualification; retain query plans, update amplification and recovery evidence. | Content CAS/history/pins, native 0.5.7 publication/eligibility/MatchUnit validation, durable relay and exact reads are merged. Public search admits 20,000 MatchUnits with a 512 raw-hit complete phrase bound, 1 MiB response cap and position/JVM-bound index audits. Fenced quarantine, exact Content replay, offline Lucene rebuild, source/index verification and new-generation activation passed in the isolated persistent fault/recovery tier of `20260924t212240-12bdb2`; the missing-body recovery run also passed. A repaired 10-Work mixed probe completed 136 reads and 32 admitted writes but returned 503 on 24 reads during concurrent graph commits. Position-change handling and the 10,000-Work mixed-load qualification remain pending, as do broader WORK10/SEARCH19/OPS05 cases. |
