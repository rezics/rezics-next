...
| Next action | Complete the practical 10,000-Work/180-second profile `load-20260925t002559-e525e2` on fixed source `c1d78a5`; it passed 6,000 sealed Works without command errors at the latest checkpoint. That source passed all seven QA tiers in `20260925t002122-58c7bb` (9 acceptance IDs passed, 20 partial, 248 uncovered). Subsequent merged source adds retained translation-link replay, MainVersion revision advancement and exact read, admitted Content publication/eligibility/comment APIs, public Main/Realm/Content paging, cmd0.5.13 native search deltas, Realm native variant recommendations, revision-disclosure checks, Access private read leases and a cmd0.5.14 native Contribution private projection candidate. The S2 real Account/Access/Main/Content API journey includes Realm adoption/rejection and an exact historical comment; it is unrun. The merged QA inventory now includes the owner-boundary Account, Access, Work and restore cases previously run separately. After load, build the cmd0.5.14 image, run its native tests and one merged QA, then repair any failures. Re-run the practical profile on the new source for the native delta/read-after-write gate. The private search HTTP route remains fail-closed pending a proven response-delivery fence. Finish Phase 0 and the retained M01–M10 backend/API gates before full web journeys. |
