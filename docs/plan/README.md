...
| Phase | S0 graph substrate verified; S1 authenticated fixed-profile Work create/edit/exact revision read, Work-scoped Access checks, an isolated graph/Access/object restore, a durable RDF outbox handoff, bounded mixed-cut Work outcome replay, one-deletion two-owner graph release guard and synchronous relay deletion journal handoff verified in part. The first S2 independently controlled text Contribution draft, private read, contributor eligibility, guarded Main Version default and Realm-local adoption/rejection, bounded Main and Realm public text search and fixed Realm-first Space creation are verified in part. Shared classification proposition and Global/Realm context profiles pass scoped model checks; distinct Realm classification context provisioning and first shared Global proposition definition pass Account/Access/Main/Fuseki and retained recovery checks. The curated Global/Realm Application and Decision profile and guarded runtime direct decisions pass scoped model, HTTP, relay and retained recovery checks. Public effective resolution and bounded classified Main/Realm phrase profiles pass scoped Global and opposite-Realm checks. The first Realm standing RatingContext passes pinned model, guarded runtime, relay and retained recovery checks; standing observations now pass scoped live and retained checks; a bounded latest-per-rater aggregate now passes scoped live and retained checks; the bounded Realm text/classification/standing-rating ARQ profile, selected-body CJK/mixed-script fixtures and bounded generation-bound public text readiness pass scoped live HTTP and retained recovery checks. S1 cross-owner recovery and historical erasure gaps remain open. S2–S3 and remaining stages are pending. |
...
| Deliverables | S0 pinned Fuseki/TDB2/jena-text drill and evidence; S1 fixed model/shape profile, guarded Main Work commands, receipts/outbox, Account/Access admission, isolated restore and recovery guards with their first consumers; first S2 text Contribution draft/profile, immutable create/edit history, contributor eligibility, guarded Main Version default and Realm-local adoption/rejection, public MatchUnit/phrase search in both contexts, private relay envelopes and retained draft/publication/selection/rejection replay; fixed Space/Realm identities, public read, typed event and retained creation replay; pinned shared classification proposition, Global/Realm context and curated decision candidate profiles; guarded typed classification context provisioning and shared Global proposition definition, public reads, relay and retained replay. Next, qualify privileged activation for rebuilt existing indexes, broaden admitted query descriptors and continue cross-owner recovery qualification. |
...
| Qualification | S0 OPS14 and substrate OPS16 passed. S1 tests passed scoped SYS02/SYS09/SYS10/SYS14, create/edit IAM07 fences, exact immutable history, a partial G3 Account/Access/Main/Fuseki HTTP path, partial OPS03/SYS13 isolated restore, deletion guard and retained journal comparison, and partial SYS04/SYS05/SYS12 outbox handoff on Fuseki 6.2.0/PostgreSQL 18.6. The first S2 private draft create/edit path, guarded contributor eligibility, Main Version default and Realm-local adoption/rejection, bounded Main and Realm public phrase search, fixed Space/Realm creation and retained draft/publication/selection/rejection/Space replay passed scoped checks. Classification proposition, context and curated decision candidates pass scoped MODEL15/17 checks; typed context and first shared proposition provisioning pass scoped live and retained runtime flows. Direct Global/Realm decisions, public effective resolution and bounded classified phrase filtering pass scoped HTTP and retained recovery checks. The Realm standing RatingContext passes scoped MODEL15/17 and runtime checks; a bounded standing aggregate passes scoped checks; the bounded Realm text/classification/rating ARQ join and selected-body CJK/mixed-script fixtures pass scoped live checks; offline CJK analyzer migration and the bounded runtime readiness gate pass scoped checks; privileged existing-index activation and broader descriptors remain pending. Complete G2–G6 and S1–S3 remain pending. |
...
MODEL15/17 candidate evidence only. Both profiles now have native graph
admission and retained replay. The first proposition command stores five
distinct identities as one Sense-owned immutable bundle. The guarded curated Application/Decision command and public effective resolver now have scoped live and retained evidence.

The updated [full HTTP result](../../services/main/tests/evidence/2026-09-24-full-work.xml)
records 402 assertions with no failures. It provisions distinct classification
...
immutable revision manifest. It also admits the first Global proposition
definition with five distinct identities, a public manifest-verified read,
denial, replay, conflict, strong cancellation and a typed private event without
the label. The updated [mixed-cut result](../../services/main/tests/evidence/2026-09-24-work-outcome-reconcile.xml)
records 324 assertions with no failures. It rejects replay without retained
...
release and returns the original key outcomes. It also rejects proposition
replay without exact retained bytes, then replays the definition and cancellation
under hold with sealed Access evidence and reads the same bundle after release.
This is scoped definition and Context identity, authority and recovery evidence. Direct decisions, effective resolution and bounded classified phrase filtering are exercised below.

The [curated classification decision candidate result](../../model/tests/evidence/2026-09-24-classification-direct-decision-profile.json)
records three conforming and six rejected Global/Realm Application and Decision
graphs under pinned Jena SHACL 6.2.0/Java 21. The profile keeps Application and
Decision identities separate, binds the Work/MainVersion target and shared
Global Sense, checks Realm context/fallback and exact revision references, and
rejects missing type, wrong context, review basis, fallback, Sense head and
unexpected predecessor. It is MODEL15/17 candidate evidence only. The installed Access-admitted guarded create/revise command stores immutable Decision history, terminal receipts and typed private relay events. The full HTTP flow verifies Global acceptance, inherited acceptance in both Realms, Realm A rejection and revision, Realm B acceptance, stale conflicts and strong cancellation. The retained mixed-cut drill replays ordered Global and Realm decisions and terminal outcomes with exact immutable manifests and current sealed Access proof. Public resolution distinguishes local, inherited, Global and absent decisions. The bounded public classified phrase profiles filter the complete text result by effective accepted state at the same graph sequence. The HTTP test covers missing, inherited, local rejected and local accepted decisions; the mixed-cut drill confirms a publication rejection still suppresses text after a classification acceptance, followed by a new-lineage Realm adoption that restores the classified match. Those earlier classified phrase profiles use position-checked additional reads; the new rated Realm profile qualifies a bounded single ARQ relation but leaves broader SEARCH01 shapes and retained reads open.

The [Realm standing RatingContext candidate result](../../model/tests/evidence/2026-09-24-realm-standing-rating-context-profile.json) records one conforming Realm/question and seven rejected type, reciprocal-link, question, grain, scale, cadence and population candidates under pinned Jena SHACL 6.2.0/Java 21. The first profile distinguishes question identity from scale numbers, fixes MainVersion as the target and counts one admitted Account principal per standing slot. The guarded runtime command now proves Account and Access admission, distinct question and Realm identities, same-key replay, immutable manifests, public reads, private typed relay, strong cancellation, and retained creation/cancellation replay under recovery hold. This is scoped context qualification; broader persona and rating policies remain pending.

The [Realm standing RatingObservation candidate result](../../model/tests/evidence/2026-09-24-realm-standing-rating-observation-profile.json) records three conforming submission, correction and withdrawal cases and eight rejected type, link, opaque slot, value, predecessor and time cases under pinned Jena SHACL 6.2.0/Java 21. It fixes one Account-principal/MainVersion/question standing slot, exact revision head, available 1–10 or valueless withdrawal and four typed event times. The guarded command now proves Account/Access admission, one opaque slot per principal/question/MainVersion, exact-head correction, a one-winner same-head race, withdrawal and restoration, private historical reads, stale and strong cancellation, and typed private relay without the principal ID. The full HTTP test includes a second Account token: the same question admits a separate observation and cannot read the first principal's history. The retained mixed-cut drill replays creation, correction, withdrawal and cancellation in source order from exact immutable manifests and current sealed Access proof. The first public latest-per-rater query now reduces at most 100 current slots with immutable manifest checks, ten score buckets, exact sum/count, explicit no-data, and source position. Live HTTP evidence covers empty, withdrawn and two-principal populations; restored withdrawal remains empty. A private historical read intermittently failed when Jena shortened a fractional `xsd:dateTime` literal; the saved failed graph demonstrated `.520Z` becoming `.52Z`. The writer now canonicalizes graph times and the reader compares instants, with a [deterministic five-assertion regression](../../services/main/tests/rating-time.test.ts) and a passing repeated HTTP run. The bounded joined Realm query now applies a one-decimal mean threshold over current standing heads in the same ARQ text/classification relation. Live HTTP evidence covers no-data, withdrawal, two-principal mean, Realm A/B classification disagreement, Realm-local adoption, source-position consistency and whole public/rating population budget errors. The joined query audits every selected-question slot for a unique, well-formed current head and returns unavailable for an incomplete head. The retained mixed-cut drill reads withdrawn no-data, then a new-lineage restoration with the expected MatchUnit and source position. Immutable manifest verification, materialized generations, broad persona rules and time import policy remain pending. The local `cjk-bigram-v1` assembler and full HTTP flow now qualify Chinese, Japanese, Korean and mixed Latin selected-body hits in one Realm, with private preselection, opposite-Realm/Main isolation, language filtering, joined classification/rating and original literal binding. The 328-assertion mixed-cut drill still passes under that assembler. The isolated offline CJK rebuild passes on disposable data. The runtime gate now compares the bounded public RDF/ index inventory and CJK probe, then binds public query results to its graph position and generation. The 402-assertion live flow checks missing marker/probe, an unindexed MatchUnit and recovery after repair; retained restore checks separate graph/text readiness under hold and after release. Privileged activation for existing rebuilt indexes, broader relevance and broader query descriptors remain open.
...
Account/Access/Fuseki/object storage. The fixed Main Version default now selects an eligible exact draft and atomically projects one public MatchUnit. A bounded complete phrase query and public selected read are locally exercised. A fixed Space command creates a public Space and distinct Realm capability with pinned initial policies. Guarded local adoption gives two Realms independent exact selected bodies, public fallback reads and complete bounded phrase queries. Local rejection/suppression is verified in the fixed profile. Management grant provisioning, broader classification Application paths, materialized rating generations, joined-query immutable-byte checks and broader query filters remain pending. The mixed-cut recovery drill verified one restored public MatchUnit and jena-text lookup; crash-time index rebuilding remains unqualified. WORK02/03, CTX01 and SEARCH01/03 remain partial until broader query paths and publication controls are qualified. The
...
| Space, Context, classification and Main Version | Selected first-stage foundation. | Fixed Space/Realm creation, Main default and Realm-local adoption/rejection, typed Realm classification context provisioning, two-Realm publication reads and bounded public phrase queries passed scoped engine/HTTP/recovery checks. Direct curated classification Applications and decisions plus bounded classified phrase filtering pass scoped live and retained checks. The first Realm standing RatingContext and current-head aggregate pass scoped checks; a bounded single ARQ Realm text/classification/rating profile and selected-body CJK/mixed-script fixtures pass live HTTP checks. Materialized aggregates, immutable-byte checks in joined queries, broader query descriptors and complete end-to-end gates remain pending. |
