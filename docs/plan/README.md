...
| [G-022](../goals/tasks/G-022.md) / RATE02 | Versioned experience Rating observation identity and exact-head revision, independent of G-020 moderation. | Ready for a fresh Rating/model worktree from merged `main`. G-015's daily profile and standing corrections establish two cadences; complete RATE02 requires an intentional experience occasion to create a distinct Observation, with correction retaining identity and real-owner/recovery evidence. Shared Main route and generated artifacts enter the manager's serial integration queue. |
