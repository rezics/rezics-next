...
| Next action | Run one full `yarn qa` on the merged cmd0.5.10/WORK02/SEARCH07/SEARCH13/SEARCH19 batch, repair any native integration failures, then run the practical 10,000-Work/180-second profile on the resulting clean source. The fixed-source 2,000-Work diagnostic `load-20260924t230845-8ae4ab` passed; its late seed rate was about 1.125 Works/s, all mixed lanes were within 1,500 ms p95, and retained-data restart passed. The preceding full QA `20260924t230001-4ab0c9` failed unit, integration and fault/recovery; its repaired failures passed the scoped `20260924t230554-dce3df` rerun, which does not certify a full run. Phase 0 and B1 remain incomplete. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
