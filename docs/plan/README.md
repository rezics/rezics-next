...
| B12 | Qualify the 10,000-Work host profile from a command-created 9,900-Work stopped baseline and a fresh 100-Work writable cohort, retaining exact owner/index identity and full 180-second mixed thresholds. | Pending. |
| B13+ | Implement and qualify each remaining retained M01–M10 backend/API dependency through stages A–G: persistence, authority, cross-service contracts, source/package workflows and operations. The [backend map](backend-acceptance.md) and owning case files select the next batch; no required API behavior remains uncovered before W1. | Pending. |
