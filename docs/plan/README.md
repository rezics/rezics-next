...
| Status | As of 2026-09-25, combined B13 recovery, Content outbox ordering and B12 search-race repairs passed all seven `yarn qa` tiers in `20260925t150116-dfb6ac`: 11 passed IDs, 35 partial and 231 uncovered. The stopped 9,900-Work source passed, but its first 10,000-Work clone failed one Content read during three concurrent native index movements; that profile is not qualified. The [qualification page](qualification.md) remains the final generated evidence owner. |
| Next action | Integrate the QA-passing branch into `main`, run the merged QA, then repeat the 10,000-Work practical profile with 100 fresh writable Works using the retained physical baseline and explicit compatible-source opt-in. Check all cold/mixed/restart thresholds. Continue unmet backend/API dependencies from the acceptance map. |
...
| B12 | Qualify the 10,000-Work host profile from a command-created 9,900-Work stopped baseline and a fresh 100-Work writable cohort, retaining exact owner/index identity and full 180-second mixed thresholds. | The clean `c47fdcc` source `load-20260925t122019-660a32` passed: 9,900 Works, 39,608 sealed Access admissions, seven exact cold cases after grant expiry, zero relay lag, hashed owner/index manifest and stopped retained volumes. Clone `load-20260925t144930-fe506d` reached 2,378 reads and 402 writes with zero writer errors, but failed one Content read with `search_index_unavailable`; its retained Main trace proved three successive native index movements, with the last 97 ms attempt rejected by the fixed attempt ceiling. Storage restart and final exact checks did not run. A bounded retry repair is in the isolated checkout; run full QA, then repeat the practical profile on a compatible source before marking B12 passed. |
| B13 | Complete the first WORK04 graph-loss recovery boundary: replay an admitted exact Work derivation from retained relay and Access evidence into the held graph, reject changed/missing evidence, preserve the relation and receipt identity, and verify idempotent duplicate replay. Owners are Main's native derivation/recovery modules, relay, the model profile and fault/recovery tests. | Full isolated-branch `yarn qa` `20260925t150116-dfb6ac` passed all seven tiers, including 14 recovery cases in 354 seconds, with 11 passed IDs, 35 partial and 231 uncovered. The single-snapshot relay repair passed real WORK04/WORK02 restore and a concurrent PostgreSQL mutation case; a 1,001-position engine test caught and verified numeric relay ordering, and a 12-event Content owner test caught the corresponding outbox ordering bug. This batch also includes B12's bounded search-race repair and explicit compatible-source load option with focused tests. Merge into `main` and run merged QA before treating B13 as integrated. |
