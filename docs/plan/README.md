...
| Next action | Run centralized `yarn qa` with the merged static gates, native command module 0.5.6, authenticated Content authoring and Main/Realm selection oracle. The preceding full run `20260924t201447-ec7e7e` passed static, integration, model, fault/recovery, browser and load; its API-contract unit failure passed in failed-only run `20260924t201815-7cd4ab` after repair. Build the two-stage text rebuild and a complete query/readiness profile beyond the current 100-unit corpus cap before the 10,000-Work load objective. P0.4 case coverage and B1 remain. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
...
| P0.2 | Java command endpoint, assembler/image wiring and transactional gate under `infra/jena/`; Main command adapter and writer migration under `services/main/`. SYS02/SYS09/SYS10/SYS14 and model validity cases are affected. | The module gate in the toolchain lock passes; Main has no `execFile` validator path. | Partial: native module 0.5.6 requires a private per-stack capability on every non-maintenance command and enforces exact in-transaction domain-head CAS for Work edit and Main/Realm selection or rejection. Main claims Access grants before dispatch. The selected native Jena and equivalence runs `20260924t201617-b8f327` and `20260924t201633-645243` passed; the merged unit tier `20260924t201919-514fd0` passed. Broader native command families and external Access binding still need qualification. |
...
| P0.4 | Shared-stack `yarn qa` core under `tests/qa/` and `scripts/qa/`, starting with real Account signup/session and Main/Fuseki readiness. OPS01/IAM01 have partial smoke coverage; later increments add oracles, split cases, model/fault/e2e/load tiers, fixture fetcher and `--record`. | `yarn qa` passes within 30 minutes on this host; setup is shared, overlapping full runs on one checkout are rejected, failure reruns preserve their partial scope, and `--record` verifies and records in one run. Any failure can be rerun by ID. | The most recent full run `20260924t201447-ec7e7e` passed all tiers except API-contract unit assertions; the failed-only run `20260924t201815-7cd4ab` passed after repair. The harness now preserves nested test selection, replays an exact property seed, and pulls one licensed Q42 fixture by committed hash with offline verification. A product-command Main/Realm selection oracle passed targeted integration `20260924t201850-ee6d96`. Scoped Biome/import gates and negative fixtures are merged; restore/crash cases, broader case coverage and a passing full clean-tree `--record` remain. |
...
| P0.8 | PostgreSQL Content revisions/drafts/receipts/outbox and preparation pins; common history adapters; exact-revision publication; bounded two-owner relay into RDF MatchUnits and embedded Lucene; coherent readiness and two-stage rebuild. | WORK02–03/09–10, SEARCH01–04/06–08/15–20 and OPS03/09/11–12/15–16 pass through the harness. Pin numeric whole-request budgets and practical mixed-load objectives before qualification; retain query plans, update amplification and recovery evidence. | Transactional Content CAS/history/pins, native 0.5.6 publication/eligibility/MatchUnit validation, durable one-event relay and authorized exact reads are merged. Authenticated `POST /v1/content-drafts` now requires exact Account/Access authority and Content CAS; eligibility independently verifies exact bytes, digest, author proof, save receipt and owner position. Strong revocation reconciles Content cancellation. The merged load tier `20260924t201037-88b9ca` passed 10 Works/12 units in 25.9 seconds. These remain partial WORK10/SEARCH19/OPS05 evidence. Two-stage rebuild and the 10,000-Work practical workload remain; the complete-population search cap is 100 units. |
