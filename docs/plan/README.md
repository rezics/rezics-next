...
| Next action | The clean merged `yarn qa` run `20260925t035859-06b5a7` passed all seven tiers on cmd0.5.15: 8 acceptance IDs passed, 30 partial, 239 uncovered. Run a command-only `yarn load --works 10 --duration 10` diagnostic for the private Contribution adapter and bounded native public delta, then repeat the practical 10,000-Work profile if it passes. The prior cmd0.5.12 profile `load-20260925t002559-e525e2` seeded 10,000 Works in 166 minutes but failed 26/2,326 mixed public reads with `503 search_index_unavailable`; the exact readiness/deadline cause remains unproved. The load runner now reserves four hours. B3a acting-context API work is ready in an isolated branch and will merge after the fixed practical load. Continue Phase 0 and retained M01–M10 backend/API gates before full web journeys. |
...
| P0.8 | PostgreSQL Content revisions/drafts/receipts/outbox and preparation pins; common history adapters; exact-revision publication; bounded two-owner relay into RDF MatchUnits and embedded Lucene; coherent readiness and two-stage rebuild. | WORK02–03/09–10, SEARCH01–04/06–08/15–20 and OPS03/09/11–12/15–16 pass through the harness. Pin numeric whole-request budgets and practical mixed-load objectives before qualification; retain query plans, update amplification and recovery evidence. | Cmd0.5.15 now validates paired Main selection and revision head transitions and passed the native build/tests plus merged QA. Public Main/Realm/Content paging, Content CAS/history/pins, translated link replay, revision reads, Realm variants, Content projection and isolated rebuild/restore have partial QA evidence. The earlier 2,000-Work diagnostic passed. The cmd0.5.12 10,000-Work profile seeded fully but failed 26 mixed public reads before restart, so practical qualification remains open. Run the cmd0.5.15 command-only diagnostic, then repeat 10,000 Works; private paging and complete P0.8 acceptance remain open. |
