...
| Next action | Harden P0.2's command ingress so callers cannot omit or underdeclare native validation and the default assembler rejects raw updates. Then expand P0.3's generated model outputs, P0.4's case-level evidence and P0.5's object adapter. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
