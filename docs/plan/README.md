...
| Phase | S0 graph substrate verified; S1 authenticated fixed-profile Work create/edit/exact revision read, Work-scoped Access checks and an isolated graph/Access/object restore verified in part. Authoritative journal reconciliation after a held older restore and full S1 cases are next. S2–S3 and remaining stages are pending. |
