...
| [G-024](../goals/tasks/G-024.md) / IAM24 | Explicit atomic move of an independent organization from one Realm episode to another admitted Realm. | Ready after one current worker handoff. G-012's separate leave/join operations demonstrate independence but not an atomic transfer; G-017 provides the separate managed-authority branch. This Access-only slice can run in a later worktree while package or Rating work continues, then be integrated serially. |
