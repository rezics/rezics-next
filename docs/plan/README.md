...
| Status | B14's first fixed native text release seal and exact-read APIs now pass the affected real API journey `20260925t165048-7dd8a1`, backend model tier `20260925t165108-1bd395`, and `yarn check:backend` on the changed source. The prior full `yarn qa` `20260925t151147-02d115` remains the last broad result: 11 passed IDs, 35 partial and 231 uncovered. The separate practical 10,000-Work profile `load-20260925t160348-90e989` passed its selected host objective, not the product scope. The [qualification page](qualification.md) remains the final generated evidence owner. |
| Next action | Complete B14's fault/recovery and complexity counterexamples, then close its affected checks and commit. Map remaining retained cases to API operations, qualify the typed persistence pilot and reusable fixture restore within 600 seconds, and proceed through B15+ without dropping backend requirements. Final reconstruction and recorded backend QA remain the final gate. |
...
| B14 | Establish WORK05's first fixed release owner path for an eligible native text selection. A separate release identity must pin an exact Main Version revision, selection, publication decision, selected draft and verified bytes under a complete bounded manifest; later Work metadata edits and default changes must not alter that release. Add admitted seal and exact read APIs with denied, stale, concurrent, idempotent, damaged-byte and restart tests, plus a derived cost contract. Owners are Main release commands/history, Access admission, generated model profile and the integration/recovery tiers. | In progress. The fixed release profile and cmd0.5.18 native binding commit one release anchor, receipt and typed outbox event. `POST/GET /v1/fixed-releases` passed denied, stale, same-key concurrent convergence, replay, changed-intent conflict, current Work disclosure, retained body after metadata/default changes, service restart and damaged manifest checks in selected real API run `20260925t165048-7dd8a1`. The new relay event was retained in that run. Backend model tier `20260925t165108-1bd395` and `yarn check:backend` passed; no full WORK05 declaration or recovery drill is claimed. Next: graph-loss/recovery and derived complexity counterexamples, then close affected checks and commit. Multi-member composition, metadata claims, availability and package resolution remain in B15+; WORK05 stays partial. |
