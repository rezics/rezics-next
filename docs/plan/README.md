...
| Deliverables | S0 pinned Fuseki/TDB2/jena-text drill and evidence; S1 fixed model/shape profile, guarded Main Work commands, receipts/outbox, Account/Access admission, isolated restore and recovery guards with their first consumers; first S2 text Contribution draft/profile, immutable create/edit history, contributor eligibility, guarded Main Version default selection, public MatchUnit/phrase search, private relay envelopes and retained draft/publication replay. Next, implement Realm-local adoption, selection recovery replay and broader qualified search; continue cross-owner recovery qualification. |
