...
| Next action | Complete the practical 10,000-Work/180-second profile `load-20260925t002559-e525e2` on clean source `c1d78a5`; it reached 1,000 sealed Works at 1.16 Works/s with no errors. That source passed all seven QA tiers in `20260925t002122-58c7bb` (9 acceptance IDs passed, 20 partial, 248 uncovered). Later Main commits add retained translation-link replay and public phrase paging; they await one merged QA after the load releases the host. Separate P0.8 worktrees are implementing bounded affected-unit search readiness and a legitimate MainVersion revision command without using the load stack. Finish Phase 0, then execute the retained M01–M10 backend/API batches before the full web journey. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
...
| P0.8 | PostgreSQL Content revisions/drafts/receipts/outbox and preparation pins; common history adapters; exact-revision publication; bounded two-owner relay into RDF MatchUnits and embedded Lucene; coherent readiness and two-stage rebuild. | WORK02–03/09–10, SEARCH01–04/06–08/15–20 and OPS03/09/11–12/15–16 pass through the harness. Pin numeric whole-request budgets and practical mixed-load objectives before qualification; retain query plans, update amplification and recovery evidence. | Content CAS/history/pins, native 0.5.12 publication/eligibility/MatchUnit and translated-Work link validation, durable relay and exact reads are merged. Public search admits 20,000 MatchUnits with a 512 raw-hit complete phrase bound, 1 MiB response cap and position/JVM-bound index audits. Fenced rebuild, WORK10 outcome recovery and SEARCH17 raw-import quarantine drills passed selected fault cases. The clean, stable 2,000-Work diagnostic `load-20260924t230845-8ae4ab` seeded in 29m15s, completed 128 reads and 22 admitted writes in 10s with zero errors, read p95 676 ms, relay lag zero and retained-data restart passing. The full 10,000-Work run is in progress and complete acceptance cases remain. WORK02 links and typed relay, SEARCH07 author joins and SEARCH19 same-language variants passed full `20260925t002122-58c7bb` as partial cases. Retained WORK02 link replay, its graph-loss case, and Main/Realm phrase paging are merged but await full QA; the graph-loss case must exercise the real native command after load. Realm recommendations, legitimate MainVersion revision, broader paging and bounded post-write readiness audits remain open. |
