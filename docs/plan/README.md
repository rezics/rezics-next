...
| Next action | Merge P0.6 locale browser verification, then run the next full `yarn qa`. P0.8 now has live exact Content publication and a fail-closed relay checkpoint; add native MatchUnit/eligibility profiles and an admitted eligibility writer before claiming public Content search. Then wire runtime/readiness/rebuild and mixed-corpus load. P0.2 maintenance authorization/domain-head guards and P0.4 case coverage remain. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
...
| P0.4 | Shared-stack `yarn qa` core under `tests/qa/` and `scripts/qa/`, starting with real Account signup/session and Main/Fuseki readiness. OPS01/IAM01 have partial smoke coverage; later increments add oracles, split cases, model/fault/e2e/load tiers, fixture fetcher and `--record`. | `yarn qa` passes within 30 minutes on this host; setup is shared, overlapping full runs on one checkout are rejected, failure reruns preserve their partial scope, and `--record` verifies and records in one run. Any failure can be rerun by ID. | Full merged run `20260924t192555-8e0769` passed static, integration, fault/recovery, built-Worker e2e and load; unit and model failed on stale profile-count/test-list expectations and a new-profile generated arbitrary. It reported 12 partial IDs and 265 uncovered. The selected integration run `20260924t192231-e874b4` passed three new paths (54 assertions). The harness unit list expectation was repaired and its selected 8-test run passed; model generator repair is in progress. Restore/crash cases, mixed-corpus load, fixture fetcher and full `--record` qualification remain. |
...
| P0.8 | PostgreSQL Content revisions/drafts/receipts/outbox and preparation pins; common history adapters; exact-revision publication; bounded two-owner relay into RDF MatchUnits and embedded Lucene; coherent readiness and two-stage rebuild. | WORK02–03/09–10, SEARCH01–04/06–08/15–20 and OPS03/09/11–12/15–16 pass through the harness. Pin numeric whole-request budgets and practical mixed-load objectives before qualification; retain query plans, update amplification and recovery evidence. | Transactional Content draft CAS, immutable revisions, exact reads, pins, receipts and outbox are merged. Jena command module 0.5.2 validates the new generated Content variant/decision profile and reciprocal receipt facts. Main's guarded publication settled a real Content pin against a native Fuseki receipt and replayed it in selected integration run `20260924t192231-e874b4`. The merged Content checkpoint/relay test proved no advance or public unit on an unproven terminal event. The relay still requires separate native MatchUnit and reviewed public eligibility profiles; no positive Content body projection or search is claimed. Runtime wiring, readiness/rebuild, rights revocation and mixed load remain. |
