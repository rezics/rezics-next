...
| [G-029](../goals/tasks/G-029.md) / RATE05 | Distinguish a new Rating question/Context from an immutable aggregate-default policy revision and real default consumer. | Recovery worker `/root/g029_recovery` owns the preserved independent Rating worktree from `5d533df`; original task `01a0dd2b-024a-7882-b412-48302629f5e7` is interrupted. The counterexample must change a context-default result from 7 to 5 without changing Context identity or relabeling explicit 7/5/4.5 policies; question edits create another Context. G-028 Work/Source is independent; shared Main/model artifacts enter the manager's serial integration queue. |
