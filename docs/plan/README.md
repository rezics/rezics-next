...
| Deliverables | S0 pinned Fuseki/TDB2/jena-text drill and evidence; S1 fixed model/shape profile, guarded Main Work commands, receipts/outbox, Account/Access admission, isolated restore and recovery guards with their first consumers; first S2 text Contribution draft/profile, immutable create/edit history, contributor eligibility decision, private relay envelopes and retained draft/publication replay. Next, implement guarded context selection and qualified public search; continue cross-owner recovery qualification. |
...
Account/Access/Fuseki/object storage. Guarded context selection and qualified public search remain the next S2 evidence
