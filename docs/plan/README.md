...
| Next action | Close P0.2 maintenance authorization and domain-head guards. P0.3's 66 native candidate matrix has 11 outcome mismatches and missing violation paths; add bounded exact bindings before retiring the historical validators. Continue P0.6 web against the merged typed Main/OpenAPI contract. Expand P0.4's fault, model, load and browser evidence alongside those consumers. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
...
| Web client | None. | — | Everything; P0.6 and B1. |
...
| P0.6 | `apps/web`: vinext on Workers, Eden with TanStack Query, Tailwind/SharkUI in `packages/ui`, `native-i18n`, Storybook, Playwright; sign-in through Account, one RSC Work page and one client search component. | Playwright on `wrangler dev` and the Storybook tests pass inside `yarn qa`. | In progress in an isolated web branch: live public search reaches Main, four Storybook browser states pass, and desktop/mobile layouts were inspected. OAuth client provisioning, authenticated Work journey, wrangler/Playwright gate and central QA registration remain. |
