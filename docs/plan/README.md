...
| Next action | Repair native command validation's whole-graph copy cost exposed by failed 10,000-Work attempt `load-20260924t221051-ccd697`, then remeasure scaling and rerun the full practical profile. In parallel, run the isolated WORK10 and SEARCH17 fault drills, merge their fixes and the new WORK03/09 and SEARCH01/02/19 tests, then run one full `yarn qa` at that batch boundary. The last clean merged run `20260924t220707-de8312` passed all seven tiers but qualified only 2 of 277 retained IDs; Phase 0 coverage and B1 remain. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
