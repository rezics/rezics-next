...
| Next action | Complete the practical 10,000-Work/180-second profile `load-20260925t002559-e525e2` running on clean source `c1d78a5`, which passed all seven full QA tiers in `20260925t002122-58c7bb` (9 acceptance IDs passed, 20 partial, 248 uncovered). Independent P0.8 worktrees are implementing retained translation-link reconstruction and bounded affected-unit search readiness without using the load stack. The fixed-source 2,000-Work diagnostic `load-20260924t230845-8ae4ab` passed with late seed rate about 1.125 Works/s, mixed read p95 676 ms, and retained-data restart. Phase 0 and B1 remain incomplete. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
...
| P0.5 | Explicit response schemas, OpenAPI export, `MainApp` type export, S3 object adapter on RustFS. | The Eden and object-storage gates pass. | Partial: aws4fetch signed conditional creation, read-back checks and eight same-key writers passed against RustFS. Main Work create/edit, exact and recovery reads use the adapter when configured. All 32 Main `/v1` routes now have explicit response schemas and inferred types; `yarn gen:check` checks `generated/openapi/main/public.json`, and the standalone Eden consumer compiles and makes two actual calls. The earlier merged static, unit and integration tiers passed in `20260924t202630-48ca4a`; the new routes await full merged QA. Other semantic object consumers, backup/restore, multipart/GC and the web Eden BFF gate remain. |
...
| P0.8 | PostgreSQL Content revisions/drafts/receipts/outbox and preparation pins; common history adapters; exact-revision publication; bounded two-owner relay into RDF MatchUnits and embedded Lucene; coherent readiness and two-stage rebuild. | WORK02–03/09–10, SEARCH01–04/06–08/15–20 and OPS03/09/11–12/15–16 pass through the harness. Pin numeric whole-request budgets and practical mixed-load objectives before qualification; retain query plans, update amplification and recovery evidence. | Content CAS/history/pins, native 0.5.12 publication/eligibility/MatchUnit and translated-Work link validation, durable relay and exact reads are merged. Public search admits 20,000 MatchUnits with a 512 raw-hit complete phrase bound, 1 MiB response cap and position/JVM-bound index audits. Fenced rebuild, WORK10 outcome recovery and SEARCH17 raw-import quarantine drills passed selected fault cases. The clean, stable 2,000-Work diagnostic `load-20260924t230845-8ae4ab` seeded in 29m15s, completed 128 reads and 22 admitted writes in 10s with zero errors, read p95 676 ms, per-lane p95 at most 580 ms, relay lag zero and retained-data restart passing. At 2,000 Works TDB2 used 12.9 GB; Fuseki anonymous memory was 1.84 GB and file cache 12.85 GB before restart. This is diagnostic evidence only; the full 10,000-Work run is in progress and complete acceptance cases remain. WORK02 links and typed relay, SEARCH07 author joins and SEARCH19 same-language variants passed full `20260925t002122-58c7bb` as partial cases. Realm recommendations, a legitimate MainVersion revision command, event-based link restore and bounded post-write readiness audits remain open. |
