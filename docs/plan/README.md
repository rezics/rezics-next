...
| Next action | Complete the practical 10,000-Work/180-second profile `load-20260925t002559-e525e2` on fixed source `c1d78a5`; it reached 2,000 sealed Works at the last report with no errors. That source passed all seven QA tiers in `20260925t002122-58c7bb` (9 acceptance IDs passed, 20 partial, 248 uncovered). Subsequent Main source adds retained translation-link replay, MainVersion revision advancement, admitted Content publication/eligibility APIs, public Main/Realm/Content paging and cmd0.5.13 native search deltas. The S2 real Account/Access/Main/Content API journey test is authored but unrun. After load, build the new image, run its native tests and one merged QA, then repair any failures. Finish Phase 0 and the retained M01–M10 backend/API gates before full web journeys. |
...
| P0.5 | Explicit response schemas, OpenAPI export, `MainApp` type export, S3 object adapter on RustFS. | The Eden and object-storage gates pass. | Partial: aws4fetch signed conditional creation, read-back checks and eight same-key writers passed against RustFS. Main Work create/edit, exact and recovery reads use the adapter when configured. All 33 Main `/v1` routes now have explicit response schemas and inferred types; `yarn gen:check` checks `generated/openapi/main/public.json`, and the standalone Eden consumer compiles and makes two actual calls. The earlier merged static, unit and integration tiers passed in `20260924t202630-48ca4a`; the new route awaits full merged QA. Other semantic object consumers, backup/restore, multipart/GC and the web Eden BFF gate remain. |
