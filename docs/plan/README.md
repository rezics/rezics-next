...
| Next action | Merge the in-flight P0.2 domain-head/Access guard, P0.8 authenticated Content provenance and P0.4 fixture worktrees, then run centralized `yarn qa`. The full `20260924t200007-44565e` run passed integration, model, fault/recovery, browser and load; its static/unit failures were repaired and both failed tiers passed in `20260924t200406-4df420`. Build the two-stage text rebuild and a complete query/readiness profile beyond the current 100-unit corpus cap before the 10,000-Work load objective. P0.4 case coverage and B1 remain. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
...
| P0.2 | Java command endpoint, assembler/image wiring and transactional gate under `infra/jena/`; Main command adapter and writer migration under `services/main/`. SYS02/SYS09/SYS10/SYS14 and model validity cases are affected. | The module gate in the toolchain lock passes; Main has no `execFile` validator path. | Partial: native module 0.5.5 retains exact bindings, bounded updates, receipt/sequence/outbox invariants and transaction validation. It now requires a per-stack private capability before bootstrap, restore cutover/release and retained-zero receipt handling; Main sends it only for those families. The targeted native 11-test run `20260924t195209-580fc9` passed before one final replay-denial assertion; the merged model tier `20260924t195652-538b84` passed after isolating fixture state. Generic exact domain-head CAS and external Access admission remain Main-owned. |
...
| P0.4 | Shared-stack `yarn qa` core under `tests/qa/` and `scripts/qa/`, starting with real Account signup/session and Main/Fuseki readiness. OPS01/IAM01 have partial smoke coverage; later increments add oracles, split cases, model/fault/e2e/load tiers, fixture fetcher and `--record`. | `yarn qa` passes within 30 minutes on this host; setup is shared, overlapping full runs on one checkout are rejected, failure reruns preserve their partial scope, and `--record` verifies and records in one run. Any failure can be rerun by ID. | Full `20260924t200007-44565e` passed integration, isolated model, fault/recovery, built-Worker browser and mixed load. Static and unit caught a web query result union and stale model test inventory; both passed in the failed-only diagnostic `20260924t200406-4df420` after repair. Native command fixtures now run in the isolated model tier. Restore/crash cases, fixture fetcher, broader case coverage and a passing full clean-tree `--record` remain. |
...
| P0.8 | PostgreSQL Content revisions/drafts/receipts/outbox and preparation pins; common history adapters; exact-revision publication; bounded two-owner relay into RDF MatchUnits and embedded Lucene; coherent readiness and two-stage rebuild. | WORK02–03/09–10, SEARCH01–04/06–08/15–20 and OPS03/09/11–12/15–16 pass through the harness. Pin numeric whole-request budgets and practical mixed-load objectives before qualification; retain query plans, update amplification and recovery evidence. | Transactional Content CAS/history/pins, native 0.5.5 publication/eligibility/MatchUnit validation, durable one-event relay and authorized exact reads are merged. Main now supervises the Content poller, fences search readiness on both owners and serves `public-content-phrase-v1` through a typed HTTP route. Live selected run `20260924t195217-6a879b` passed publication → claimed eligibility → projection → exact public phrase route. Isolated mixed load `20260924t195624-7e03bb` seeded 10 Works/12 units and made 336 requests in 20 seconds with zero HTTP failures and 30.5 ms p95. These remain partial WORK10/SEARCH19/OPS05 evidence. Authenticated Content authoring, independent rights provenance, revocation, two-stage rebuild and the 10,000-Work practical workload remain. The present complete-population search cap is 100 units. |
