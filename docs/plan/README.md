...
| Status | As of 2026-09-25, merged B4 passed full `yarn qa` in `20260925t085124-7fa328`: all seven tiers passed, with 9 passed IDs, 32 partial and 236 uncovered. This qualifies the changed batch snapshot, not the retained product scope. Both 10,000-Work load attempts remain failed. The [qualification page](qualification.md) remains the final generated evidence owner. |
| Next action | Integrate staged B5 as the next backend dependency slice, reconcile its new group authority, native query and load-preparation paths with current cost contracts, then run merged `yarn qa`. Preserve B6–B7 for later dependency order. Replace repeated command-only background seeding with compatible verified fixtures. |
...
| B5 | Extend Stage A IAM05/IAM36 with private same-scope Agent group inheritance for `work.create`; qualify SEARCH04 rated-relation multiplicity and SEARCH06 CJK Main/Realm selection; add an optional bounded seed-worker profile for OPS05/SEARCH18/SEARCH19 diagnostics. Owners are Access schema/context/admission, Main native query fixtures and the practical-load runner. Derive and check path costs, bounds and denied/stale outcomes. | Branch `codex/b5-backend-integration` is staged after B4. Reconcile it with qualified B4, run one central `yarn qa`, repair its failure queue and record its scoped result. The parallel seed option defaults to one worker; a small A/B diagnostic precedes any full-profile use. |
| B6–B7 parked | Existing work on `codex/b6-backend-integration` and `codex/b7-backend-integration`. | Dependent work is staged, not accepted capability. Preserve the branches and integrate one coherent dependency slice at a time after B5 passes. Do not infer qualification from branch names or commit counts. |
