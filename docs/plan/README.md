...
| Next action | P0.1's image, Compose stack and root commands are integrated; complete its clean-clone exit by P0.2's removal of Main's host Jena/Java/Python validator path. Start the runnable P0.4 harness core on the merged stack so one existing Main/Account path has recorded shared-setup evidence. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
...
| P0.4 | Current increment: shared-stack `yarn qa` core under `tests/qa/` and `scripts/qa/`, starting with an existing real Main/Account path; later increments add oracles, split cases, model/fault/load tiers, fixture fetcher and `--record`. IAM01/IAM07/SYS02 and OPS01 are affected first. | `yarn qa` passes within 30 minutes on this host; setup is shared, overlapping full runs on one checkout are rejected, failure reruns preserve their partial scope, and `--record` verifies and records in one run. Any failure can be rerun by ID. | Runnable core in progress in a disjoint worktree. Coordinator wires root commands and runs the merged batch. Next: first shared-setup path and compact run record; remaining tiers stay pending. |
