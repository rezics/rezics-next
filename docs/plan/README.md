...
| [G-028](../goals/tasks/G-028.md) / LIVE03 | Owner-enforced Work title editorial-control epoch and explicit source-control return. | Ready after G-025's merged Rating/model verification releases a worker slot. Existing source adoption, refresh assessment, guarded title application and equal-value human/source CAS supply foundations, but full source/human control and generic protection remain unqualified. The brief keeps exact owner CAS, distinct return authority, real recovery and bounded cost in scope; G-026 npm is independent. |
