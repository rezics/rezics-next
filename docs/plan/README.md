...
| Next action | Merge the P0.8 native model/Jena profile and bounded Content-to-Main relay branches, update Main's command-module readiness pin, then run the next full `yarn qa` after the P0.6 browser repair. The Content publication boundary is merged and fails closed while its native profile is absent. P0.2 maintenance authorization/domain-head guards, P0.4 case coverage and mixed-corpus load remain. The selected PostgreSQL + Jena/TDB2/jena-text/Lucene architecture remains in force. |
...
| P0.4 | Shared-stack `yarn qa` core under `tests/qa/` and `scripts/qa/`, starting with real Account signup/session and Main/Fuseki readiness. OPS01/IAM01 have partial smoke coverage; later increments add oracles, split cases, model/fault/e2e/load tiers, fixture fetcher and `--record`. | `yarn qa` passes within 30 minutes on this host; setup is shared, overlapping full runs on one checkout are rejected, failure reruns preserve their partial scope, and `--record` verifies and records in one run. Any failure can be rerun by ID. | Partial: merged run `20260924t185900-d7549a` passed static, unit, integration, native model, fault and empty-corpus load; e2e failed because public search asserted sequence zero after the authenticated test had created a Work. The selected e2e repair run `20260924t190214-c33442` passed, including 10 Storybook browser checks. The last full run had 11 partial IDs and 266 uncovered. Restore/crash cases, mixed-corpus load, fixture fetcher and full `--record` qualification remain. |
...
| P0.6 | `apps/web`: vinext on Workers, Eden with TanStack Query, Tailwind/SharkUI in `packages/ui`, `native-i18n`, Storybook, Playwright; sign-in through Account, one RSC Work page and one client search component. | Playwright on `wrangler dev` and the Storybook tests pass inside `yarn qa`. | Built Worker, Eden BFF, public search, RSC Work metadata, sign-in/studio shell and Storybook are merged. The isolated authenticated Playwright run `20260924t185618-ef975f` passed sign-in/PKCE, invalid callback, denied actor and Work creation receipt with desktop/mobile review. The selected QA e2e repair `20260924t190214-c33442` passed browser selection and 10 Storybook tests. A full merged pass and broader locale/authority states remain. |
| P0.7 | README, installation, service READMEs and model README describe the actual commands. | `yarn docs:check` passes; the documented commands run as written. | Root README, installation, operations index, Main/Account service READMEs and model README now document the current commands and separate the S0 raw-update drill from the product command path. Merged `yarn docs:check` passed 166 Markdown files and 9 checker tests. The later Content publication and authenticated browser commands need a final pass after those boundaries land; `yarn dev` with the new web workspace still needs a merged runtime check. |
| P0.8 | PostgreSQL Content revisions/drafts/receipts/outbox and preparation pins; common history adapters; exact-revision publication; bounded two-owner relay into RDF MatchUnits and embedded Lucene; coherent readiness and two-stage rebuild. | WORK02–03/09–10, SEARCH01–04/06–08/15–20 and OPS03/09/11–12/15–16 pass through the harness. Pin numeric whole-request budgets and practical mixed-load objectives before qualification; retain query plans, update amplification and recovery evidence. | Transactional Content draft CAS, immutable revisions, exact reads, pins, receipts and outbox are merged and passed the QA integration tier in `20260924t185318-e7e20f`. A guarded Main publication command and exact terminal receipt reconciliation are merged; its PostgreSQL/simulated-graph test passed 26 assertions in the module branch. It fails closed until the new native Jena profile lands. Live relay/search, readiness/rebuild and load remain. |
