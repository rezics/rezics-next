...
| Status | As of 2026-09-25, merged B5 passed full `yarn qa` in `20260925t091518-584f03`: all seven tiers passed, with 9 passed IDs, 32 partial and 236 uncovered. This qualifies the changed batch snapshot, not the retained product scope. Both 10,000-Work load attempts remain failed. The [qualification page](qualification.md) remains the final generated evidence owner. |
| Next action | Merge and review staged B6's Account erasure restore, represented Work proof and exact Work derivation against their owner contracts and cost bounds. Run central `yarn qa` and close its failure queue before B7. Replace repeated command-only background seeding with compatible verified fixtures. |
...
| B6 | Extend M01 and native Work boundaries: IAM11 older/current Account and Access physical restore against retained deletion evidence; IAM33 exact represented `work.create` proof across mandate, grant, Agent and principal generations; WORK04 explicit adaptation, recording and fork derivation from an exact retained source revision. Owners are Account, Access, Main, graph/model and the integration/fault tiers. | Staged on `codex/b6-backend-integration` for review and central QA after B5. The Work profile requires the documented cmd0.5.16 Fuseki image. IAM11, IAM33 and WORK04 retain broader unqualified paths. |
| B7 parked | Existing work on `codex/b7-backend-integration`. | Dependent work is staged, not accepted capability. Integrate after B6 passes. |
...
| W1+ | Build and qualify the full web product journeys against the completed APIs, including the authenticated S2 flow and all retained frontend acceptance. Finish local full-application browser verification and the clean final `yarn qa --record`. | Waiting for B1–B8+ backend/API gates. |
