...
| Next action | Implement a compatible isolated background fixture with a small real-command cohort before another 10,000-Work attempt. First use the retained two `search_index_unavailable` Content reads and the 100/1,000-Work diagnostics to choose and falsify the fixture path and a bounded read-race reproducer. Continue unmet backend/API dependencies from the acceptance map. |
