...
| B5a | Extend M01 Access with private same-scope Agent group inheritance for `work.create`; IAM05/IAM36 first profile, bounded activation and selected-path command claim. Owners are Access schema, private context/admission, Account/Access/Main owner fixture and identity contract. | Isolated branch `codex/b5-group-hierarchy` stages migration 013, group mutation/proof boundary, current context and admission checks, and an owner fixture. Runtime verification waits for the ongoing practical load and coordinator QA. Populated reparenting is denied; public management, impact approval, general roles, principal membership and wider profile qualification remain open. |
| B5+ | Implement and qualify each remaining retained M01–M10 backend/API dependency through stages A–G: persistence, authority, cross-service contracts, source/package workflows and operations. The [backend map](backend-acceptance.md) and owning case files select the next batch; no required API behavior remains uncovered before W1. | Pending. |
| W1+ | Build and qualify the full web product journeys against the completed APIs, including the authenticated S2 flow and all retained frontend acceptance. Finish local full-application browser verification and the clean final `yarn qa --record`. | Waiting for B1–B5+ backend/API gates. |
