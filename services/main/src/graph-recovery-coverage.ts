...
if (process.argv[2] !== 'capture' || !fusekiUrl || !accountUrl || !accessUrl || !relayUrl
  || !consumer || !key) {
  throw new Error('usage: FUSEKI_URL=... ACCOUNT_RECOVERY_DATABASE_URL=... ACCESS_RECOVERY_DATABASE_URL=... RELAY_RECOVERY_DATABASE_URL=... RELAY_CONSUMER=... RECOVERY_MANIFEST_HMAC_KEY=<64 hex characters> bun graph-recovery-coverage.ts capture');
}
