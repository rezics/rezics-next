...
export type NpmSnapshotRequest = NpmRequest | NpmPlatformRequest | NpmIdentityRequest | NpmCompositionRequest | NpmPolicyRequest;
type NpmSnapshotOutcome = NpmOutcome | NpmPlatformOutcome | NpmIdentityOutcome | NpmCompositionOutcome | NpmPolicyOutcome;
...
export class NpmResolutionStore {
  ...
  private verified(row: Row): NpmResolution {
    try {
      ...
      return { profile: row.request.profile === 'npm-lock-v3-topology-v5' ? 'npm-lock-topology-receipt-v5'
