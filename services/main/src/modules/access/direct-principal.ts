...
export async function directWorkCreateProof(client: PoolClient, principalId: string,
  ...
  const granted = await client.query<{ id: string; generation: string }>(`
    ...
    ORDER BY id LIMIT 1 FOR SHARE`, [principalId]);
  const attributed = await client.query<{ id: string; generation: string }>(`
    SELECT a.id, a.generation FROM access.principal_agent_attribution a
    ...
    ORDER BY a.id LIMIT 1 FOR SHARE OF a, s`, [principalId, actingSubject]);
