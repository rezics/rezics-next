...
export async function groupWorkCreateProof(client: PoolClient, subject: string,
  ...
  const result = await client.query<{
    ...
    SELECT p.member_id, grant.id AS grant_id, p.depth, p.parent_id, p.cycle
    FROM path p LEFT JOIN LATERAL (
      ...
      ORDER BY id LIMIT 1
    ) grant ON true ORDER BY p.member_id, p.depth, grant.id`,
...
export async function selectedGroupWorkProof(client: PoolClient, subject: string,
  ...
  ) SELECT grant.id FROM path p JOIN access.group_permission_grant grant
    ON grant.group_id = p.id WHERE grant.id = $3 AND grant.scope_id = $4
      AND grant.action = 'work.create' AND grant.active
      AND grant.valid_until > clock_timestamp() LIMIT 1`,
