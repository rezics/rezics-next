...
export async function readWorkTerminalReceipt(
  ...
  const result = await fuseki.query(`PREFIX rv: <${RV}>
    SELECT ?outcome ?digest ?admissionId ?authorityEpoch ?scope ?work ?main ?sequence ?epoch WHERE {
      GRAPH <${RECEIPTS}> {
  ...
  if (outcome === 'succeeded' && (!row.work || !row.main)) {
    throw new Error('successful Work receipt has no result');
  }
  if (outcome === 'cancelled' && (row.work || row.main)) {
