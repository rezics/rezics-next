...
async function validateCandidate(env: WorkActivationEnvironment, selection: string,
  slot: string, input: SelectRealmLocalInput, draft: string): Promise<void> {
  mkdirSync(env.candidateDirectory, { recursive: true, mode: 0o700 });
  const temp = mkdtempSync(join(env.candidateDirectory, 'realm-selection-'));
  try {
    const data = join(temp, 'candidate.ttl');
    writeFileSync(data, `@prefix rv: <${RV}> .\n` +
      `${iri(selection)} a rv:PublicationSelection ; rv:context ${iri(input.context.id)} ; ` +
      `rv:slot ${iri(slot)} ; rv:work ${iri(input.work)} ; ` +
      `rv:mainVersion ${iri(input.mainVersion)} ; rv:contribution ${iri(input.contribution)} ; ` +
      `rv:publicationDecision ${iri(input.publicationDecision)} ; ` +
      `rv:selectedDraft ${iri(draft)} ; rv:selectionBasis rv:RealmManagerReview ; ` +
      `rv:selectionMode rv:Fixed ; rv:reviewPolicy ${iri(REVIEW_POLICY)} .\n`,
    { mode: 0o600 });
    const { stdout } = await execFileAsync(env.python, [
      join(env.repositoryRoot, 'model/tools/validate_realm_local_selection.py'),
    ...
    if (report.conforms !== true || !report.profile_sha256) {
      throw new Error('Realm selection candidate validation incomplete');
    }
...
export async function selectRealmLocal(env: WorkActivationEnvironment,
  ...
  await validateCandidate(env, selection, slot, input, exact.revision);
